export default {
    settings: {
      ...
      gatewayRuntime: {
        title: '网关运行时设置',
        description: '调整网关运行中的超时与使用日志明细保留策略',
        responseHeaderTimeout: '响应头超时时间（秒）',
        responseHeaderTimeoutHint: '等待上游返回响应头的最长时间。',
        responseHeaderTimeoutWarning: '修改过短可能导致长时间排队的上游请求提前失败。',
        openAITextFirstTokenTimeout: '生文请求首 Token 超时时间（秒）',
        openAITextFirstTokenTimeoutHint: '仅影响 OpenAI Responses 流式请求；0 表示关闭。',
        openAIImageFirstTokenTimeout: '生图请求首 Token 超时时间（秒）',
        openAIImageFirstTokenTimeoutHint: '仅影响明确生图的 OpenAI Responses 流式请求；0 表示关闭。',
        streamDataIntervalTimeout: '流数据间隔超时时间（秒）',
        streamDataIntervalTimeoutHint: '0 表示关闭；启用时范围为 30 到 300 秒。',
        usageLogDetailRetentionLimit: '普通使用日志明细保留上限',
        usageLogDetailRetentionLimitHint: '每个用户保留的普通请求明细数量；0 表示不保留。',
        imageUsageLogDetailRetentionLimit: '图片使用日志明细保留上限',
        imageUsageLogDetailRetentionLimitHint: '每个用户保留的图片请求明细数量；0 表示不保留。',
        loadFailed: '加载网关运行时设置失败',
        loadFailedInline: '网关运行时设置加载失败，成功加载前无法保存。',
        saved: '网关运行时设置已保存',
        saveFailed: '保存网关运行时设置失败',
        validationFailed: '网关运行时设置包含无效值',
      },
      features: {
        channelMonitor: {
          title: '渠道监控',
