...
const defaultBaseUrl = computed(() => {
  const platform = props.account?.platform ?? ''
  // CN 供应商 / OpenCode：按当前模式/协议回落到官方预设（清空输入框提交时使用），
  // 不能落到 anthropic 默认值（会被当 CC base 拼出错误端点）。
  if (isCNProviderPlatform(platform) || platform === 'opencode_go') {
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    const platformDefaultUrl = (isCNProviderPlatform(newAccount.platform) || newAccount.platform === 'opencode_go')
      ? defaultCNBaseUrl(newAccount.platform, currentOpenCodeOrCNMode(), editApiProtocol.value)
      : getDefaultBaseUrl(newAccount.platform)
...
function applyOpenAIExtra(extra: Record<string, unknown>) {
  ...
  // Runtime state is service-owned and must not be overwritten by account edits.
  ...
  if (props.account.type === 'oauth' || props.account.type === 'setup-token') {
    ...
    // Claude Code 插件放行已迁移到全局 codex_cli_only_whitelist。
