...
describe('AccountUsageCell', () => {
  ...
  it('OpenAI OAuth 有现成快照时，手动刷新信号会触发 usage 重拉', async () => {
    ...
    expect(getUsage).toHaveBeenNthCalledWith(1, 2010, undefined)
    expect(getUsage).toHaveBeenNthCalledWith(2, 2010, undefined)
  ...
  it('OpenAI OAuth 在行数据刷新但仍无 codex 快照时会重新拉取 usage', async () => {
	...
	await flushPromises()
	expect(getUsage).toHaveBeenCalledTimes(2)
	expect(getUsage).toHaveBeenNthCalledWith(1, 2003, undefined)
	expect(getUsage).toHaveBeenNthCalledWith(2, 2003, undefined)
