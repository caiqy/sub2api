    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'm2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0L21.75 15.75m-16.5-9h13.5A1.5 1.5 0 0 1 20.25 8.25v7.5a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5v-7.5a1.5 1.5 0 0 1 1.5-1.5Zm4.5 3h.008v.008H9.75V9.75Z'
        })
      ]
    )
}
...
const flagVisibleUserPayment = () => flagPayment() !== false && authStore.user?.hidden_purchase_page !== true
...
function buildSelfNavItems(withDashboard: boolean, respectHidden: boolean): NavItem[] {
  const items: NavItem[] = []
  if (withDashboard) {
  ...
  items.push(
    ...
    { path: '/images', label: t('nav.aiImages'), icon: ImageIcon },
    ...
    { path: '/purchase', label: purchaseNavLabel.value, icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: respectHidden ? flagVisibleUserPayment : flagPayment },
    { path: '/orders', label: t('nav.myOrders'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/redeem', label: t('nav.redeem'), icon: GiftIcon, hideInSimpleMode: true },
    { path: '/affiliate', label: t('nav.affiliate'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagAffiliate },
    { path: '/profile', label: t('nav.profile'), icon: UserIcon },
    ...(respectHidden ? customMenuItemsForUser.value : customMenuItemsForUserUnfiltered.value).map((item): NavItem => ({
      path: `/custom/${item.id}`,
      label: item.label,
      icon: null,
...
const userNavItems = computed((): NavItem[] => finalizeNav(buildSelfNavItems(true, true)))
...
const customMenuItemsForUser = computed(() => {
  ...
  return items
    ...
    .filter((item) => !isCustomMenuHidden(item.id, authStore.user?.hidden_custom_menu_ids))
    .sort((a, b) => a.sort_order - b.sort_order)
})

const customMenuItemsForUserUnfiltered = computed(() => {
  const items = appStore.cachedPublicSettings?.custom_menu_items ?? []
  return items
    .filter((item) => item.visibility === 'user')
    .sort((a, b) => a.sort_order - b.sort_order)
...
const adminNavItems = computed((): NavItem[] => {
  ...
  if (authStore.isSimpleMode) {
    ...
    filtered.push({ path: '/images', label: t('nav.aiImages'), icon: ImageIcon })
    filtered.push({ path: '/batch-image', label: t('nav.batchImage'), icon: BatchImageIcon, featureFlag: flagBatchImageAccess })
