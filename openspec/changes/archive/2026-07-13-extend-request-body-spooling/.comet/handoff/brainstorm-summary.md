...
- Anthropic Messages 路由到 Antigravity 时，handler 只借出 Claude effective handle；Antigravity 同步转换后创建并独占 Gemini payload handle，内部 retry loop 只保存 handle 和小型 metadata，每个网络 attempt 通过 `Open`/`GetBody` 重开。
...
- 不接受 Antigravity 转换后 payload 的内存例外；同步转换可以短暂 materialize，但 payload bytes 不得跨网络等待或 retry 生命周期。
...
- Antigravity：真实 Messages handler 的 gzip 10MB+ 阻塞上游、Gemini payload hash、retry replacement、usage/billing/session、503 与双层 handle cleanup。
