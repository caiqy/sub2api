...
- SQL review: the added 077/078/079/130/149/159/174/175/175a/176 files use guards, replaceable functions, drop-if-exists triggers, or convergent updates as appropriate. `108_add_group_user_concurrency.sql` is the exception: its two `ADD COLUMN` statements lack `IF NOT EXISTS`; the runner prevents a second normal application, but manually replaying that SQL after a partial/manual schema change is not idempotent. This is a warning, not a filename/order/checksum conflict.
- Focused migration tests passed: `go -C backend test ./migrations -count=1` (`ok`, `0.505s`). This covers embedded migration content and the added long-context, API-key index, and Grok provider regression checks; no database integration migration replay was run.

### Handoff

- Task20 may consume this evidence for the complete capability matrix. Warning retained: repair the historical 108 SQL only through a new forward migration if an affected deployment needs partial-schema recovery; do not edit its checksum-protected file.
