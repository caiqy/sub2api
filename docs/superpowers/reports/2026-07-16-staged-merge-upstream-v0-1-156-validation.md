...
| OpenAI previous-response/session Sticky | OpenAI 开关关闭时不读写 response/session/WS 状态；开启时保持绑定、权重与清理语义。 | `OpenAIGatewayService.Forward -> SelectAccountWithScheduler -> sticky store`。 | `openai_account_scheduler*.go`、`openai_ws_*`。 | 152, 153, 155, 156 | `cd backend && go test ./internal/service -run '^(TestOpenAIGatewayService_SelectAccountWithScheduler_PreviousResponseStickyDisabledBypassesStickyLookupAndBind\|TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyDisabledBypassesLookupBindAndRefresh\|TestOpenAIGatewayService_StickyDisabledWSv2SkipsStateStore)$' -count=1` | HTTP、WS V2、ingress 三入口共享同一平台开关。 | protected | 阶段 0 PASS；四段均 protected。 | `platform-sticky-boundaries`；CodeGraph context/explore。 |
| Gemini/Anthropic Sticky | 按最终平台而非 Anthropic alias 判定开关；禁用时不读写或清理会话。 | `GatewayService.stickyEnabledForPlatform -> get/bindStickySessionForPlatform -> Gemini handler/compat`。 | `gateway_service.go`、`gateway_helper.go`、`gemini_sticky_toggle_test.go`。 | 153, 156 | `cd backend && go test ./internal/handler -run '^(TestGatewayHandler_GeminiRouteStickyLookupUsesGeminiToggleNotAnthropicToggle\|TestGatewayHandler_GeminiRouteStickyBindUsesGeminiToggleNotAnthropicToggle\|TestGatewayHandler_OpenAIRouteStickyUsesOpenAIToggleNotAnthropicToggle)$' -count=1` | Antigravity retry cleanup 必须遵从 Anthropic 开关。 | protected | 阶段 0 PASS；153、156 protected；152、155 未直接触及该调用链。 | `platform-sticky-boundaries`；CodeGraph explore；附录 B、D。 |
...
| OpenAI 首 Token 超时 | 仅流式 Responses 按明确 image intent 选择 watchdog；超时不可 failover/处罚账号。 | OpenAI handler -> `openai_first_output_timeout` -> SSE/WS forwarder。 | `openai_first_output_timeout.go`、`openai_first_token_http_test.go`、`openai_ws_*`。 | 156 | `cd backend && go test ./internal/service ./internal/handler -run '^(TestOpenAIFirstTokenTimeoutResponsesChatFallbackBeforeHeaders\|TestResponsesChatFallbackAppliesStreamDataIntervalAfterFirstOutput\|TestOpenAIGatewayHandler_ResponsesDecodesGzipRequestBody)$' -count=1` | 仅在 v0.1.156 合并后执行批准移除/替换判定；前三段不得提前移除保护。 | approved-removal | v0.1.152-v0.1.155 protected；仅 v0.1.156 后可执行。 | `openai-first-token-timeout`；本节 changed-files。 |
