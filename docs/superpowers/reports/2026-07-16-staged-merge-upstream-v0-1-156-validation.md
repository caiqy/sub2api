...
- `git log --oneline --ancestry-path "v0.1.156^{}..HEAD"` begins with the v0.1.156 merge and otherwise contains only local Task 14-22 compatibility repairs, tests, coordination, and canonical evidence commits. No post-release upstream commit appears. `git merge-base --is-ancestor upstream/main HEAD` returned the required nonzero exit `1`, so `upstream/main` is not a result ancestor.
- The 154-commit first-parent review separates the four immutable merges from their documented ordinary repair/test commits and preparation/completion coordination. The final Task 19-21 documentation commits each touch only this canonical report; no unexpected merge, unresolved index entry, push, release, deployment, or merge-to-main was found. `git status --short --branch` still shows only the pre-existing untracked `.comet/current-change.json`.
