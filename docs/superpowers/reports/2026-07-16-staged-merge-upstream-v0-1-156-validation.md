...
## Task 16 First-Token Removal Residue Verification

- Exact old-symbol scan: `openai_text_first_token_timeout|openai_image_first_token_timeout|first_token_timeout|gateway\.openai_first_token_timeout|OpenAIFirstTokenTimeout|openAIFirstTokenWatchdog|openAIFirstToken|FirstTokenTimeout|ResponsesFirstToken|ResponsesEventEndsFirstTokenWait|ResponsesEventRecordsFirstToken` found `0` business-code matches under `backend`, `frontend`, and `deploy`; no compatibility key, error, log event, or watchdog was restored.
- Historical classification: the same scan found `38` OpenSpec-document matches, `15` canonical-report matches, and `103` other documentation-history matches. They describe the approved removal or archived prior behavior only; Task 16 does not modify OpenSpec, plan, or history.
- Conflict scan: `git grep -n -E '^(<<<<<<< .+|=======|>>>>>>> .+)$' -- . ':!docs/superpowers/reports/**'` and `git diff --name-only --diff-filter=U` both returned no matches.
...
## Task 18 v0.1.156 Stage Gate Completion

- Baseline: fresh validation ran from `00c1cef38f78182305596d608d5cf2a6e5fa986b` (`fix: restore credential failover contract`). The only worktree entry before and after the gate was the pre-existing untracked `.comet/current-change.json`; it was not modified.
- Required full gate: `make test` passed, including backend default and unit suites, `golangci-lint`, frontend lint/typecheck, and frontend Vitest (`181` files, `1405` tests). `pnpm --dir frontend run build` passed, transforming `970` modules. Existing Browserslist, dynamic-import, and chunk-size warnings remained warnings with exit code `0`.
- Capability rerun: M-01 through M-15 all passed. The named coverage counts were `2 + 4 + 4 + 2 + 3 + 4 + 4 + 4 + 4 + 4 + 5 + 3 + 18 + 0 + 2`; the M-06 untagged service command still has zero matching tests and its two assertions passed under `-tags unit`; M-14 is compile-only/manual by design.
- Timeout and follow-up rerun: retained `FirstOutputTimeout|HandleStreamTimeout`, configured WebSocket client-first-message timeout, `openai_ws_v2`, native failed-response usage, terminal de-duplication, and single/bulk probe checks all passed. No local first-token watchdog or compatibility alias was restored.
- Generation and static checks: `make -C backend generate` completed; the restricted Ent/Wire diff was empty. Fresh `go test ./... -run '^$' -count=1`, legacy first-token business-path scan, conflict-marker scan, `git diff --check`, unmerged-index checks, and target-tag ancestor checks all passed. The first-parent merge sequence remains `v0.1.152`, `v0.1.153`, `v0.1.155`, then `v0.1.156`.
- Non-gate observation: root `make build` was attempted once on Windows and failed because `make.exe` invokes `cmd.exe` while `backend/Makefile` requires POSIX `sh` and `CGO_ENABLED=0` shell syntax. Task18's fixed commands are `make test` plus `pnpm --dir frontend run build`; this extra command is not a gate, was not repaired, and is not a blocker.
- Boundary: no production, test, generated, VERSION, OpenSpec, plan, Comet, merge, push, release, or deployment file changed. Task19 remains unstarted and retains ownership of the version decision.

## Task 19 Metadata, Generation, and Migration Review

### Result and version

- Result: `PASS with warning`. `v0.1.156` is the highest three-part tag contained by the pre-task result. `backend/cmd/server/VERSION` was therefore changed from the carried `0.1.155` to `0.1.156.1` in the standalone commit `75726cf468faa7080f631c790e5f8a083abb8bae` (`chore: set version to 0.1.156.1`). No tag, release, push, deploy, generated source, plan, OpenSpec, or Comet state changed.
- `go build -o <temp>/sub2api-task19.exe ./cmd/server` passed. Executing `sub2api-task19.exe --version` printed `Sub2API 0.1.156.1 (commit: unknown, built: unknown)`. `cmd/server/main.go` embeds `VERSION`, trims it during `init`, and accepts a non-empty ldflags `Version` override.

### Generation and metadata provenance

- Round 1 and round 2 each ran `make -C backend generate`, followed by `git diff --exit-code -- backend/ent backend/cmd/server/wire_gen.go`; both restricted diffs were empty. No generated-output instability was found.
- `git diff --name-status v0.1.151..HEAD` found only the staged-release metadata set: `VERSION`; `go.mod`/`go.sum`; `frontend/package.json`/`pnpm-lock.yaml`; `deploy/config.example.yaml`; and the migration additions below. The final version bump is the sole Task19 production-file change.
- Go metadata promotes existing `golang.org/x/mod v0.35.0` to a direct requirement and adds its transitive checksum entries. Frontend metadata adds `vue-eslint-parser@^9.4.3`; the lockfile importer resolves `9.4.3(eslint@8.57.1)`. Neither was changed by Task19.
- Deploy defaults are release/required-compatibility changes, not Task19 changes: v0.1.156 supplies the native OpenAI output timeout, client-first-message timeout, and token-refresh bounds; v0.1.153-v0.1.155 supplies Server-Timing, image non-stream keepalive, direct `x/mod`, and the web-search migration. Defaults remain explicit and compatibility-safe (`false`/`0` where disabled).

### Migration review

- `git ls-tree -r --name-only HEAD backend/migrations` reports `226` SQL files. The v0.1.151..HEAD additions, recorded by complete filename, are: `077_add_usage_log_details.sql`, `078_add_usage_log_detail_upstream_request.sql`, `079_add_usage_log_detail_upstream_response.sql`, `108_add_group_user_concurrency.sql`, `130_add_usage_log_detail_type.sql`, `149_content_moderation_matched_keyword.sql`, `159_user_resource_overrides.sql`, `174_add_usage_log_long_context_billing.sql`, `174_add_usage_logs_api_key_latest_ip_index_notx.sql`, `174_group_web_search_price_per_call.sql`, `175_add_ops_system_logs_host.sql`, `175_default_openai_long_context_billing.sql`, `175a_add_ops_system_logs_host_index_notx.sql`, and `176_channel_monitor_grok_provider.sql`.
- There are no duplicate full filenames. `41` numeric prefixes intentionally have multiple lexical variants: `006x2, 028x3, 029x2, 033x2, 034x2, 036x2, 037x2, 045x3, 046x2, 047x2, 052x2, 053x2, 054x2, 060x2, 070x2, 071x2, 075x2, 077x2, 078x2, 079x2, 081x2, 095x2, 101x4, 102x2, 108x2, 125x2, 126x2, 127x2, 130x2, 134x2, 135x2, 136x3, 145x2, 149x2, 151x2, 154x2, 158x2, 159x2, 160x2, 174x3, 175x2`. The new tails execute lexically as `174_add_usage_log_long_context_billing`, `174_add_usage_logs_api_key_latest_ip_index_notx`, `174_group_web_search_price_per_call`, `175_add_ops_system_logs_host`, `175_default_openai_long_context_billing`, `175a_add_ops_system_logs_host_index_notx`, then `176_channel_monitor_grok_provider`.
- `ApplyMigrations -> applyMigrationsFS` acquires a PostgreSQL advisory lock, creates/aligns migration state, calls `fs.Glob("*.sql")`, applies `sort.Strings(files)`, and processes each full filename. Existing `schema_migrations(filename, checksum)` entries are checksum-verified then skipped; changed SQL fails closed. `*_notx.sql` is statement-split and run outside a transaction; all other files run with SQL and its checksum insert in one transaction. Thus normal reruns are idempotent at the runner level.
- SQL review: the added 077/078/079/130/149/159/174/175/175a/176 files use guards, replaceable functions, drop-if-exists triggers, or convergent updates as appropriate. Follow-up `3b7db2664` changes only the two 108 `ADD COLUMN` clauses to `IF NOT EXISTS`, so partial schemas with one existing column now converge on either direct execution.
- The controlled historical checksum rule for `108_add_group_user_concurrency.sql` accepts only the released checksum `4782d26e6ced99a1221ee2fa20471dc412d4e3cfa9aaad4830940077a4c40163` and current checksum `64fbcb154429bd550b5bdfb2ad4fe18b4162bbc292dd017b98d46548461e3cc4`. The compatibility test reads the embedded current file, requires the latter checksum, verifies exactly this two-value set, and rejects an unknown value.
- Focused migration tests passed: `go -C backend test ./migrations -count=1` (`ok`, `0.712s`) and `go test ./internal/repository -run 'Test(ApplyMigrations|IsMigrationChecksumCompatible|MigrationChecksumCompatibilityRules)' -count=1` (`ok`, `1.574s`). The integration-tag test creates an empty isolated database with a partial `groups` table, executes the embedded 108 SQL twice, and asserts both columns. It was not run because this environment has no `docker` command and the integration harness requires Docker; no integration PASS is claimed.

### Handoff

- Task20 may consume this evidence for the complete capability matrix. This review does not authorize further migration, tag, release, push, or deploy changes.

## Task 20 Final Capability Matrix Review

### Scope, Stage Links, And Classification

- Scope: the current tree after the four immutable merges `4ffe039a` (v0.1.152), `9219483d` (v0.1.153), `347ad613` (v0.1.155), and `94a681bbd` (v0.1.156), plus the compatibility and gate repairs recorded in [v0.1.152 / Task 6](#stage-v152-review), [v0.1.153 / Task 9](#stage-v153-review), [v0.1.155 / Task 12](#stage-v155-review), [v0.1.156 / Task 17](#stage-v156-review), [Task 18](#task-18-v0156-stage-gate-completion), and [Task 19](#task-19-metadata-generation-and-migration-review). All four tag peels are ancestors; first-parent order is `152 -> 153 -> 155 -> 156`.
- Classification remains `protected=11`, `manual=4`, `approved-removal=1`, `gap=0`. `manual` means that an automated assertion is retained where available but cannot replace the listed cross-layer or generated-artifact review. No row is accepted by default, and no unexplained or pending capability conflict remains.
- Stage terms: `PASS` means the stage changed-file intersection and its targeted/final evidence are closed; `N/A` means that stage did not directly change the row's entry chain, not that it was skipped. The final Task18 rerun covers M-01..M-15 and the retained timeout paths.

| ID / final class | Current entry symbol / path | Key call chain and retained boundary | v0.1.152 / v0.1.153 / v0.1.155 / v0.1.156 | Final command, count, or manual evidence | Repair SHA(s) |
| --- | --- | --- | --- | --- | --- |
| M-01 Scheduler / protected | `OpenAIGatewayService.SelectAccountWithScheduler`, `selectAccountWithScheduler` in `backend/internal/service/openai_account_scheduler.go`; layered `Select` in `openai_account_scheduler_layered.go` | gateway forward -> scheduler `Select` -> previous-response/session layers -> `selectByLayeredFilter` -> `newSelectionResult(..., waitPlan)` -> upstream; capability/privacy/recheck precede forward. | PASS / PASS / PASS / PASS | `go test ./internal/service -run '^(TestLayered_WaitPlanFallbackSkipsUpstreamRestrictedAccount|TestOpenAISelectAccountWithLoadAwareness_AllFullWaitPlan)$' -count=1`: PASS, 2; Task18 M-01 rerun. | `7364aba7e`, `a0778e9a4` |
| M-02 OpenAI Sticky / protected | `OpenAIGatewayService.openAIStickyEnabled`; `SelectAccountWithScheduler`; WS ingress `ProxyResponsesWebSocketFromClient` in `backend/internal/service/openai_ws_forwarder_ingress.go` | Forward/Responses/WS -> sticky lookup/bind -> scheduler -> DB recheck; disabled OpenAI sticky bypasses previous-response, session, WS V2 and ingress state-store reads/writes. | PASS / PASS / PASS / PASS | unit scheduler command: PASS, 4 named targets; Task18 M-02 rerun. | `12292a77a` |
| M-03 Gemini/Anthropic Sticky / protected | `GatewayService.stickyEnabledForPlatform` in `backend/internal/service/gateway_service.go`; `GeminiMessagesCompatService.stickyEnabledForPlatform` in `gemini_messages_compat_service.go` | Gateway helper/compat -> platform-specific lookup or bind -> cache; Gemini uses `StickyGeminiEnabled`, Anthropic/Antigravity use `StickyAnthropicEnabled`, so aliases cannot cross-enable state. | N/A / PASS / N/A / PASS | tagged handler/service command: PASS, 4 named targets; Task18 M-03 rerun. | `12292a77a` |
| M-04 fallback / WaitPlan / protected | `OpenAIGatewayHandler` failover loops -> `HandleFailoverError` in `backend/internal/handler/failover_loop.go`; Responses `openAIForwardMayFailover` | selection may return WaitPlan; pre-write failures can switch; written SSE blocks ordinary failover. Native first-output may replay only after its explicit safe-write classification and replay check. | PASS / PASS / PASS / PASS | handler command: PASS, 2; retained first-output/failover follow-up and Task18 rerun passed. | `0502b26d1`, `00c1cef38` |
| M-05 DB recheck / protected | `layeredOpenAIAccountScheduler.selectBySessionHash` and `selectAccountByPreviousResponseIDForCapability` in `backend/internal/service/openai_account_scheduler*.go` | sticky/cache ID -> repository hydration -> transport/model/image/privacy/runtime/rate-limit recheck -> selection or request-scoped fallback before upstream. Binding deletion is limited to binding-invalid cases. | PASS / N/A / PASS / PASS | service command: PASS, 3 named targets; Task18 M-05 rerun. | `12292a77a` |
| M-06 protocol conversion and passthrough / protected | `GatewayService.ForwardAsChatCompletions` in `backend/internal/service/gateway_forward_as_chat_completions.go`; corresponding `ForwardAsResponses` | route -> `ForwardAs*` -> apicompat bridge -> `buildUpstreamRequestWithSourceBody(source,effective)` -> upstream. Tool schema/usage conversion remains; passthrough maps/forwards read raw client body rather than mapped body. | PASS / PASS / PASS / PASS | apicompat PASS, 2; tagged service passthrough PASS, 2; Task18 M-06 rerun records the untagged `0 tests to run` only as non-coverage. | `a8eded776`, `773731199`, `27fca813` |
| M-07 terminal usage / protected | `OpenAIGatewayHandler.Responses` -> `handleStreamingResponseWithReasoning` in `backend/internal/service/openai_gateway_response_handling.go` -> `submitFailedUsageLog` | stream bridge records terminal/usage once; native `response.failed` commits only after downstream flush, preventing generic fallback duplication; partial failover retains one failed usage. | PASS / PASS / PASS / PASS | apicompat/handler command: PASS, 4 named targets; native failed usage follow-up PASS. | `fed8873b8`, `764566330`, `e82078cf8` |
| M-08 privacy and content moderation / protected | `ExtractContentModerationInput` in `backend/internal/service/content_moderation_input.go`; `ContentModerationService` in `content_moderation.go` | handler request -> bounded audit extraction -> moderation config/group lookup -> decision/log; audit preview does not mutate the forwarding body; deleted groups are dropped and lookup failures remain visible. | N/A / N/A / PASS / PASS | service command: PASS, 4 named targets; Task18 M-08 rerun. | `1bf78df3` |
| M-09 image capability / protected | `IsImageGenerationIntent` / `IsImageGenerationIntentMap` in `backend/internal/service/image_generation_intent.go`; `SelectAccountWithSchedulerForImages` | endpoint/body intent -> required image capability -> scheduler/recheck -> image rate-limit/failover. Non-image requests do not inherit image capability or cooldown constraints. | PASS / N/A / PASS / PASS | service command: PASS, 4 named targets; Task18 M-09 rerun. | `6ed1a4541`, `acba7d6c` |
| M-10 runtime hot settings / protected | `SettingService.GetGatewayRuntimeSettings` / `SetGatewayRuntimeSettings` in `backend/internal/service/setting_gateway_runtime.go`; `SettingsView.buildGatewayRuntimePayload` | admin DTO/API -> persisted setting -> config runtime snapshot/invalidation -> new gateway requests; retained timeout fields are deployment config, not restored local first-token settings. | PASS / PASS / PASS / PASS | config command: PASS, 4 (`default-off`, env, minimum, WS first-message); frontend retained settings suite is in M-13/Task18. | `3d0c8eb24`, `9d0000963`, `5272f1da9` |
| M-11 request-body replay and cleanup / protected | handler `requestBodyCoordinator` in `backend/internal/handler/request_body_coordinator.go`; `BindOpenAIRequestBodyHandle` in `backend/internal/service/openai_gateway_request_body.go` | handler raw/effective/OAuth handles -> forward/failover -> deferred coordinator and handle cleanup. Raw/effective bodies survive a retry and owned spool/multipart files are released on success, error, and early return. | PASS / PASS / PASS / PASS | handler/service command: PASS, 5 named targets; Task18 adds 9 body-size matrix cases with matching SHA-256 and cleanup. | `fed8873b8`, `95c1ca744`, `6ed1a4541`, `b6ca703e7` |
| M-12 user resource controls / manual | `AdminService.UpdateUser` in `backend/internal/service/admin_user.go`; `PaymentHandler.GetCheckoutInfo`; `PageHandler.GetPageContent` | admin resource write -> user override state -> payment/page authorization; frontend hides the same resource, but direct backend access is rejected independently. | PASS / N/A / N/A / PASS | tagged service/handler command: PASS, 3 named targets; manual cross-layer review closed. | retained local contract; no staged-release repair required |
| M-13 frontend local functions / manual | `frontend/src/router/index.ts` and `App.vue` -> `frontend/src/utils/userUiVisibility.ts` -> `isCustomMenuHidden`; `SettingsView.vue` runtime form | route guard/direct navigation and navigation components use the visibility helper; runtime card uses the API DTO. UI hiding is paired with M-12 backend denial. | PASS / PASS / PASS / PASS | `pnpm --dir frontend test:run ...feature-access...userUiVisibility...SettingsView.gatewayRuntime...`: PASS, 3 files/18 tests; Task18 full Vitest PASS. | retained local contract; no staged-release repair required |
| M-14 version and dependencies / manual | `backend/cmd/server/main.go` init reads `backend/cmd/server/VERSION`; `backend/go.mod`, `go.sum`, `frontend/package.json`, `pnpm-lock.yaml`, `deploy/config.example.yaml` | build/version endpoint -> embedded version; release metadata is reviewed against tag history. No application capability silently depends on a version mismatch. | PASS / PASS / PASS / PASS | Task19: `go build ... ./cmd/server` PASS; `--version` is `0.1.156.1`; manual dependency/config review recorded. | `75726cf46` |
| M-15 Ent, Wire, and migrations / manual | Ent schemas -> `make -C backend generate` -> `initializeApplication` in `backend/cmd/server/wire_gen.go`; `ApplyMigrations -> applyMigrationsFS` in `backend/internal/repository/migrations_runner.go` | generated graph is reproducible; migration runner lexically sorts full filenames, checks `(filename, checksum)`, and fails closed on unknown changed SQL. The 108 compatibility accepts exactly released/current checksums. | PASS / PASS / PASS / PASS | two generate/restricted-diff rounds empty; migration unit PASS (`226` SQL files, `41` duplicate prefixes, no duplicate filename); repository compatibility PASS. | `baa9bf698`, `3b7db2664` |
| M-16 local first-token timeout / approved-removal | retained HTTP: `openAIFirstOutputTimeout` and `handleStreamingResponseWithReasoning`; client WS ingress: `ProxyResponsesWebSocketFromClient`; timeout accounting: `RateLimitService.HandleStreamTimeout` | Retained native HTTP first-output has config `openai_first_output_timeout_seconds`, default `0` (off), high-effort override, `first_output_timeout`, safe failover and `HandleStreamTimeout`. Retained client WS first-message timeout is `gateway.openai_ws.client_first_message_timeout_seconds`, plus existing WS read/write timeouts. Approved non-retention: no equivalent upstream WebSocket *upstream-output* watchdog; local pooled/V2 cancel-drain watchdog is deliberately absent. | protected / N/A (protected) / N/A (protected) / approved-removal | old-symbol business scan: 0; retained service/handler/WS tests PASS; frontend typecheck PASS; Task18 timeout rerun PASS. | `a9a95d5d7`; retained HTTP failover `0502b26d1` |

### M-16 Approval Boundary

- This is the sole approved removal. It removes only the local text/image first-token configuration, runtime persistence/DTO/API/UI, `first_token_timeout` error/log/usage contract, HTTP SSE watchdog, and upstream WebSocket output watchdog; no compatibility alias is retained.
- It does retain the upstream native HTTP first-output semantics: default-off deployment configuration, high-reasoning-effort override, `first_output_timeout`, safe failover, and `HandleStreamTimeout`. It separately retains the client-side WebSocket first-message deadline and ordinary WebSocket read/write deadlines.
- It deliberately does **not** claim an upstream replacement for the former upstream WebSocket output watchdog. Its absence is the informed approved removal, not a default acceptance or an unexplained regression.

### Task 19 Metadata And Residual Verification
...
- Static checks were clean: `git diff --check`, `git diff --cached --check`, clean cached index, `git diff --name-only --diff-filter=U`, and `git ls-files -u` exited `0`. The exact worktree and cached conflict-marker `git grep` commands each exited the expected `1` for no matches. The old first-token business-symbol scan over `backend`, `frontend`, and `deploy` found `0` matches.
...
## Task 22 Final Git Topology And Review Boundary

- Fixed peel verification: `b73d8c3efe01a290eaaa9326b6e40ece02c67a0e` (`v0.1.152^{}`), `a2bc1337474b68b62391116835e5698ebb5526bd` (`v0.1.153^{}`), `41cec0db059ffb82d0efdcfcf07a24ab51fbfe97` (`v0.1.155^{}`), and `12f991dde8a58e183d4bd16a87ef6fd0df714757` (`v0.1.156^{}`) each returned exit `0` from `git merge-base --is-ancestor <peel> HEAD`.
- The complete first-parent merge output for `d1cc02502271f54b3b7f0593a18db4f2aaab63ea..HEAD` contains exactly four merges, in strict order: `4ffe039a4399f8cbac1f83df32b709afda777ffe` (`f10199795fd5fe4ef54c99553149177612179756`, `b73d8c3efe01a290eaaa9326b6e40ece02c67a0e`) for v0.1.152; `9219483d7c34606e7c2cb530c00a46b764096414` (`4e4ed09887bfbe9e8072ea60b137b85f704da185`, `a2bc1337474b68b62391116835e5698ebb5526bd`) for v0.1.153; `347ad61301c989a6ea53ba2539022513879aceab` (`2ec17fce2f923d237336b9ebb56793553d5cf5d8`, `41cec0db059ffb82d0efdcfcf07a24ab51fbfe97`) for v0.1.155; and `94a681bbdad61f2d0bef089e14ed214c83f411da` (`d4820cd1b8952bd1ff61d61110055c614b680eba`, `12f991dde8a58e183d4bd16a87ef6fd0df714757`) for v0.1.156. Thus every second parent is the required fixed peel.
- `git log --oneline --ancestry-path "v0.1.156^{}..HEAD"` lists newest commits first, ends with the v0.1.156 merge, and otherwise contains only local Task 14-22 compatibility repairs, tests, coordination, and canonical evidence commits. No post-release upstream commit appears. `git merge-base --is-ancestor upstream/main HEAD` returned the required nonzero exit `1`, so `upstream/main` is not a result ancestor.
- The 154-commit first-parent review uses the pre-documentation baseline ending at `2e47c0913`; it separates the four immutable merges from their documented ordinary repair/test commits and preparation/completion coordination. The final Task 19-21 documentation commits each touch only this canonical report; no unexpected merge, unresolved index entry, push, release, deployment, or merge-to-main was found. `git status --short --branch` still shows only the pre-existing untracked `.comet/current-change.json`.
- `git diff --check d1cc02502..HEAD` repeats the already documented inherited v0.1.156 upstream warning at `backend/internal/service/openai_gateway_grok_test.go:2159` (`new blank line at EOF`, exit `2`). It is outside the Task 22 topology gates and no Git history was modified. Task 22 result: `PASS`.

## Task 23 Final Documentation Closure

- This closure was written before its own final documentation commit; `docs: verify staged upstream merge v0.1.156` was pending at writing. It does not report a push, release, deployment, merge to `main`, Docker integration, or archive as complete.
- Fixed evidence is recorded in [固定对象与工作区](#固定对象与工作区): Comet base `d1cc02502271f54b3b7f0593a18db4f2aaab63ea`, implementation base `d5f8192d32d9840d63477c24d4a567abb8cb4a90`, and the four immutable tag peels `b73d8c3efe01a290eaaa9326b6e40ece02c67a0e`, `a2bc1337474b68b62391116835e5698ebb5526bd`, `41cec0db059ffb82d0efdcfcf07a24ab51fbfe97`, and `12f991dde8a58e183d4bd16a87ef6fd0df714757`. The complete per-stage changed-files stdout is retained in [正式证据附录（五份原始 stdout）](#正式证据附录五份原始-stdout).
- Conflict ledgers and the corresponding merge SHA/parent tuples are retained in Task 5 (`4ffe039a`), Task 8 (`9219483d`), Task 11 (`347ad613`), and Task 14 (`94a681bbd`). Every ordinary repair/test SHA is retained in its owning stage review: Task 6, Task 9, Task 12, Task 15/17, and the complete Task 18 remediation groups; no repair was folded into a merge commit.
- Gate chain is closed by stage 0, Task 7 (v0.1.152), Task 10 (v0.1.153), Task 13 (v0.1.155), Task 18 (v0.1.156), and the final local non-Docker gate in Task 21. Task 20 records the final matrix as `protected=11`, `manual=4`, `approved-removal=1`, `gap=0`; Task 19 records `VERSION=0.1.156.1`, dependency/default provenance, two stable Ent/Wire generations, and migration ordering/checksum evidence.
- The sole approved removal is M-16: all local first-token settings, persistence/DTO/API/UI, error/log/usage contract, HTTP SSE watchdog, and upstream WebSocket output watchdog are absent with no compatibility alias. Upstream native HTTP first-output (default-off, high-effort override, `first_output_timeout`, failover, `HandleStreamTimeout`) and client WebSocket first-message/read/write timeouts remain; no replacement for the removed upstream WebSocket output watchdog is claimed.
- Task 22 proves all fixed peels are ancestors, exactly four first-parent merges occur in `v0.1.152 -> v0.1.153 -> v0.1.155 -> v0.1.156` order, and `upstream/main` is not an ancestor. Docker-backed migration integration remains unexecuted because Docker is unavailable; the inherited v0.1.156 EOF whitespace warning and non-gate Windows `make build` incompatibility remain recorded risks.
