...
- Task 25 / OpenSpec 7.3 专项审查以 `0ce1457573545383c698f9265504b230e0415a9e` 为任务起点，最终源码/test HEAD 为 `dda0a2e72`；生产 ownership chain 为 `dd58e6f6a`、`518bd5ff1`、`6b560cd1f`、`e63a6d380`、`dda0a2e72`，`e437a9e5b` 只是初版 doc commit。fresh review 证明 handler 预测 channel/account/OAuth 模型不具权威性，`dd58e6f6a` 改由 forwarding 层报告实际 outbound model。本会话未提供 CodeGraph MCP，以下是精确源码、调用边和命名测试证据，不声称 CodeGraph trace。
...
- `dda0a2e72` 补齐 HTTP/relay terminal ownership。普通 HTTP Responses 的 sensitivity RED 为 `TestOpenAIGatewayHandler_ResponsesFailedUsageUsesAccountMappedModel` 预期 `gpt-account`、实际 `gpt-5.4-high`；恢复 attempted-only reset 后 GREEN。WS full reset 仅进入 Responses WebSocket account attempt，不进入普通 HTTP Responses 或 Messages loop。relay completion 从“观察到 upstream terminal”后移到 terminal downstream `WriteFrame` 成功后；写失败不发布 success 或释放 turn permit，adapter final path 以 turn 1 error 单次调用 `AfterTurn(1,nil,err)`。`TestRelayOnTurnCompleteWaitsForTerminalDownstreamWrite` 与 relay metrics focused command 均 GREEN。
...
- 最新验证：`dda0a2e72` focused handler、service 与 `internal/service/openai_ws_v2` 矩阵均 exit 0；`go test ./internal/handler -count=1` exit 0（63.812s），`go test ./internal/service -count=1` exit 0（104.229s）。此前 OAuth image HeapAlloc 单次失败仍作为 Task 26 residual，原因 UNKNOWN；本轮未复现不等于已接受例外或消除风险。
