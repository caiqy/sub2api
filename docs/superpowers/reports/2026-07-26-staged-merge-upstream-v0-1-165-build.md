...
- Task 25 / OpenSpec 7.3 专项审查以 `0ce1457573545383c698f9265504b230e0415a9e` 为任务起点，以 `e437a9e5b` 为审查源码 HEAD；随后 fresh review 发现 WS failed-usage 模型快照缺口，修复提交为 `be9b9023b`。本会话未提供 CodeGraph MCP，以下是精确源码、调用边和命名测试证据，不声称 CodeGraph trace。
...
- 普通 OpenAI HTTP 仍采用 `ChatCompletions/Responses -> newJSONRequestBody -> ReadRaw -> SetUsageRequestBody -> SetEffectiveBytes -> BindOpenAIRequestBodyHandle -> Forward*`。Chat 在 handler 中把 `effectiveBody` 传给 `ForwardAsChatCompletions`；该 service 入口先通过 `openAIRequestBodyBytes` 读取已绑定的可回放 handle，再解析模型并派生或注入 compatibility `prompt_cache_key`，最后以该 source body 构造上游请求。Live 是独立 SDP/session 控制路径，不注入 Chat/Responses prompt-cache 语义。
- `requestBodyCoordinator` 在入口冻结 JSON/multipart raw body；SHA 相同的 effective body 复用 raw handle，派生 effective/OAuth/multipart handle 可回放，`Cleanup` 去重释放 handle 并删除 multipart 临时文件。`TestRequestBodyCoordinator_CleanupGinTerminationPaths` 覆盖成功、拒绝、取消、panic 和上游 4xx/5xx；本轮 `TestOpenAIGatewayHandler_ResponsesDecodesGzipRequestBody`、四个 Responses spool/read-error 测试及 Responses 压缩/大 body failover replay 测试均通过。
- `UsageDetailCapture` 在 handler 前生成不可变 detail snapshot。Live 的 `finalizeLiveCall` 用 `MarkLiveCallClosed` 保证只释放一次租约并写一条零 token `RequestTypeLive` usage。HTTP/WS failed usage 则在入队前快照 detail、channel 字段和精确上游模型；`be9b9023b` 使 WS 首帧先按 channel mapping、再按 account mapping 冻结模型，后续帧在 account mapping 后冻结，避免异步 failed usage 回退到 client model。`TestOpenAIResponsesWebSocketFailedUsageFreezesChannelThenAccountMappedModel` 的 RED 为期望 `gpt-account`、实际 `gpt-client`，修复后 GREEN。
