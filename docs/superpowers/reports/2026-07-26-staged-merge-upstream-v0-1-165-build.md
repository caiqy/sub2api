...
| shutdown/Cleanup/background drain | signal -> CloseAdmission -> graceful shutdown/force close -> handler drain -> `app.Cleanup()` -> producers -> usage -> ops/deferred -> quota -> billing -> Redis -> Ent。 | `I`：`main.go`；stage-only：`shutdown.go`、`wire.go`、lifecycle tests 和 deferred drain。 | Task 19 的 cmd/server targets 与 deferred target PASS。 | hard deadline 或 phase deadline 时下游 teardown 会跳过；这是有界停机取舍，不声称零丢失。 | `protected` |
...
| axios manifest/lockfile | release source declares `axios ^1.18.0` and lock importer pins `1.18.1` with integrity/dependency snapshot. | `I`：`frontend/package.json`、`frontend/pnpm-lock.yaml`。 | Git tag diff plus current manifest/lock stanzas; `pnpm list` diagnosis showed a stale ignored local install, not tracked source. | `frontend/node_modules/.pnpm/lock.yaml` remains at v0.1.162/1.16.0 locally; a fresh frozen install is outside this pure-evidence task. | `protected` |
...
| Task 17 final Sol provenance | Task 17's final code/test evidence is retained above; the binding brief records merge/review closure. | No reviewer session transcript or explicit final Sol PASS is present in the permitted formal ledger sections. | Not automatable from source; no reviewer was requested. | External reviewer provenance remains a manual coordination fact, separate from the six behavior contracts. | `manual` |
...
- 汇总：`protected=6`，`manual=1`，`gap=0`，`approved-removal=0`。唯一环境 concern 是 ignored frontend installation 仍是 axios `1.16.0`；它不改变 committed manifest/lock 结论。
