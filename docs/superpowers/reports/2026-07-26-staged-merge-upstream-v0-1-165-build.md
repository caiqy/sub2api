...
- `make SHELL=D:/scoop/shims/bash.exe build` exit 0；仍记录 Windows resolver `CreateProcess(... sh ...) failed` 使 linker 版本参数为空的环境 concern。主树 generate 命中 `user-mapped section` 后，短 detached worktree `D:\w21` 在同一提交上连续两次 generate/Ent-Wire diff 均为零并已清理；未手改生成物。静态检查无 unmerged/精确 marker，VERSION `0.1.159.6`，禁用 timeout marker 为 0。
