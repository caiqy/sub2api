...
- 起始 branch/HEAD 已按要求核验为 `feature/20260726/staged-merge-upstream-v0-1-165` / `489fa1025de715d85c6368cc96680d8de1801eec`；CodeGraph MCP 在本 agent 不可用，以下为源码读取和命名测试证据，不声称 CodeGraph trace。
- Path A（OpenAI/Grok-compatible composite）：`gateway.go:431` 的 `EffectiveGatewayRouteResolver.Resolve` -> `gateway.go:445` 的 `ApplyEffectiveGatewayRoute` -> `openai_chat_completions.go:153` 的 effective request platform -> line 193 的 `SelectAccountWithSchedulerForCapability` -> `openai_account_scheduler.go:2386` 的 `getOpenAIAccountSchedulerWithContext` -> line 2471 的 `scheduler.Select`。该路径的实际命令为 `go test ./internal/server/routes -run '^(TestCompositeTargetPlatformMiddleware(AppliesFallbackBeforeProtocolDispatch|LoadsFinalSubscription|ResolvesModelAndRestoresBody|UsesExplicitRouteAndRewritesBody|UsesExplicitRouteForMultipartImages|RejectsOversizedRuntimeRouteModels)|TestGatewayRoutesComposite(MessagesWithGrokModelUsesOpenAIGateway|ChatCompletionsWithGrokModelUsesOpenAIGateway|OpenAIOnlyEndpointsRequireOpenAITarget))$' -count=1 -v`（9 top-level PASS）和 `go test ./internal/service -run '^(TestResolveCompositeRouteDecisionRecomputesForDifferentEffectiveGroup|TestResolvePlatformUsesEffectiveGroupCompositeDecision|TestCompositeGroupSchedulerHasAllCanonicalPlatformBuckets|TestCompositeRouteResolverExplicitRoutesCoverBucketTwoProviders|TestOpenAIGatewayService_SelectAccountWithScheduler_(Weighted_PreservesGrokSessionSticky|SessionStickyDBRuntimeRecheckSkipsStaleCachedAccount|DBFreshGroupRecheckReleasesMovedAccount|LayeredRequirePrivacySet)|TestLayered_(GroupedAccountPassesDBFreshRecheck|SessionStickyPreservesGrokBinding|WaitPlanFallbackSkipsUpstreamRestrictedAccount|FallbackWaitPlanRechecksPrivacyRequirementAgainstDB)|TestAdvancedScheduler(SharesProbeBudgetWithFallbackDBRechecks|ReleasesSlotWhenDBDisablesCandidate|ReacquiresOnceWhenDBConcurrencyChanges))$' -count=1 -v`（15 top-level PASS），覆盖 composite effective route、advanced/layered、Grok sticky、WaitPlan 与 fresh DB recheck。
- Path B（generic Gateway load-aware composite）独立于 Path A factory：`GatewayService.SelectAccountWithLoadAwareness` 在 `gateway_scheduling.go:113-117` 执行 `checkClaudeCodeRestriction`/`withGroupContext`，line 118 的 `resolvePlatform` 解析 composite route，lines 135-145 进入其自身 platform sticky，再走 load-aware selection、`AccountWaitPlan` 与 scheduler-snapshot/account recheck；不经过 `getOpenAIAccountSchedulerWithContext` 或 `scheduler.Select`。实际命令为 `go test ./internal/service -run '^(TestResolvePlatformUsesEffectiveGroupCompositeDecision|TestGatewayService_SelectAccountWithLoadAwareness|TestSelectAccountWithLoadAwareness_UsesFallbackGroupForChannelRestriction|TestSelectAccountWithLoadAwareness_StickyReadReuse|TestSelectAccountWithLoadAwareness_LoadBatchDisabledUsesPrefetchedGeminiStickyAccount|TestSelectAccountWithLoadAwareness_StickyDisabledBypassesPrefetchAndCacheRead|TestOpenAIGatewayService_SelectAccountWithLoadAwareness_DBFreshGroupRecheckWaitsOnValidAccount)$' -count=1 -v`（5 top-level + 3 nested PASS）及 `go test -tags=unit ./internal/service -run '^(TestGatewayService_SelectAccountWithLoadAwareness|TestGatewaySelectAccountWithLoadAwareness_HydratesSelectedAccountFromSchedulerSnapshot)$' -count=1 -v`（2 top-level、其中 generic load-aware 28 nested PASS）。
- Handler bridge evidence for Path A was also run as `go test ./internal/handler -run '^(TestGatewayHandlerResolveStickyRouteRecomputesCompositeFallbackDecision|TestGatewayHandlerResolveEffectiveGatewayRouteUsesConcreteClaudeCodeFallback|TestOpenAIGatewayHandler_MessagesUsesEffectiveClaudeCodeFallbackGroup|TestOpenAIGatewayHandler_CountTokensUsesEffectiveClaudeCodeFallbackGroup|TestOpenAIResponsesWebSocketResolvesCompositeExplicitAliasOnFirstFrame|TestOpenAIResponsesWebSocketResolvesCompositeExplicitAliasOnEveryFrame|TestOpenAIResponsesWebSocketAppliesAccountMappingAfterLaterCompositeRoute|TestOpenAIResponsesWebSocketRejectsLaterCrossProviderCompositeRoute|TestOpenAIResponsesWebSocketRejectsLaterCompositeNoRoute|TestOpenAIResponsesWebSocketClosesOnCompositeResolverError|TestOpenAIImages_OAuthTextIsReleasedBeforeBlockedUpstream)$' -count=1 -v`（9 top-level PASS）。Ollama/admin/Alipay 8、migration package 20 也已通过；本机 migration integration 因 Docker unavailable guard 未执行 cases，不作为 PASS。
...
| Path A OpenAI/Grok-compatible | `ApplyEffectiveGatewayRoute -> gateway dispatch -> ChatCompletions -> openAICompatibleRequestPlatform -> SelectAccountWithSchedulerForCapability -> selectAccountWithScheduler`；advanced disabled 时 factory 返回 nil 并进入 `selectAccountWithLoadAwareness`；enabled 时 layered 构造 layered scheduler，weighted/default/unknown fallback 构造 default scheduler，只有非 nil scheduler 才调用 `scheduler.Select`。平台参数进入该分支，未绕过 Grok/session sticky。 | Task 21 `TestOpenAIGatewayService_SelectAccountWithScheduler_(Weighted_PreservesGrokSessionSticky|SessionStickyDBRuntimeRecheckSkipsStaleCachedAccount|DBFreshGroupRecheckReleasesMovedAccount|LayeredRequirePrivacySet)`、`TestLayered_(...)`、`TestAdvancedScheduler(...)`，15 top-level PASS。 | protected |
...
- fresh review 独立修复：merge `dc3df2d57` 保持纯净。`ca4ec7452038181d64002ec5d1a5831e24801f8d fix: fail closed on saturated email alias probes` 修复非 Gmail dotted SQL 候选过匹配填满旧 `Limit(50)` 而遗漏后置真实 plus alias 的信任边界 fail-open。RED 饱和回归先在旧查询上返回 false；修复查询 `limit + 1`，超过 50 候选即经原 bool API 返回 true/fail closed。精确 RED、GREEN、全部 alias repository tests、repository compile-only 与 `git diff --check` 均有记录；不改变 migration、VERSION、lockfile 或 Task 24/25 边界。
...
- Task 24 / OpenSpec 7.2 closure（final archive HEAD `e98ce7a784023a400221ce71f859de1f0708ff4a`）：migration integration 现固定要求 12/12 上游 files、两个 `186_*` 和 `190_add_users_email_alias_dedup_index_notx.sql`，同时保留并验证本地/上游同号 172、181。`make test` PASS（Vitest 215 files / 1626 tests），显式 `VERSION=0.1.159.6` build PASS，二进制和 VERSION 均为 `0.1.159.6`；短路径 detached worktree 中两轮 Ent/Wire generate 均 PASS 且零 diff。最终 `local-serv-ai`（Go `1.26.5`、Docker `29.2.1`）以隔离三变量 temp 执行完整 `CI=true GOFLAGS='-v' go test -tags=integration ./...` exit 0，空库 migration PASS（4.86s）、从 local v0.1.159.6 升级 PASS（4.91s），无 FAIL。唯一 OAuth Images memory RED 是 test fixture 自身保存 20 MiB 输入而使全 package heap delta 不稳定；生产 `body = nil` 尝试被 lint 证明无效并精确 revert，最终改为流式 fixture，保留 12 MiB release assertion。远程目录/archive 已清理，日志在 `%LOCALAPPDATA%/Temp/opencode/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2-integration.log`；13 个 skip 均为既有环境/config 项且不涉及 migration/repository target。Task 25 Live 与 prompt cache/body replay 专项审查、最终 VERSION 和 Task 26 final verify 仍待执行。
- Task 24 fresh-review evidence correction：远程 source archive 为 `e98ce7a784023a400221ce71f859de1f0708ff4a`，local tar `C:/Users/caiqy/AppData/Local/Temp/opencode/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2.tar`，唯一目录 `/tmp/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2`；解包并重建 `src/backend/.test-tmp` 后执行 `CI=true GOFLAGS='-v' TMPDIR='/tmp/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2/src/backend/.test-tmp' TMP='/tmp/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2/src/backend/.test-tmp' TEMP='/tmp/sub2api-task24-final-9d6e7f1a4b234acf8da9b07d58e4f6c2/src/backend/.test-tmp' go test -tags=integration ./... > integration.log 2>&1`，ssh-skill execution JSON 为 `success=true, exit_code=0`。日志中 `TestMigrationsRunner_IsIdempotent_AndSchemaIsUpToDate` PASS（4.86s）、`TestMigrationsRunner_UpgradesLocalV01596AcrossUpstreamStages` PASS（4.91s），无 `--- FAIL:`/`FAIL`；12/12 来自 `20648b826` 的固定集合断言加升级 target PASS，而非成功路径日志打印。13 个 skip 已逐项核验：`TestDingTalkOAuthStart_Disabled`（DingTalk disabled）、`TestDialerAgainstCaptureServer`（external TLS capture）、`TestConcurrencyCacheSuite/TestGetAccountsLoadBatch`（known CI TODO）、`TestPromptAuditConfigCASSecretRoundTripInvalidationAndTTL`、`TestRedisPayloadStoreRoundTripTTLNamespaceAndDelete`、`TestPromptRuntimeAggregatesConfigWorkersQueueRedisEndpointsAndGuardMetrics`、`TestPromptAuditMigrationSchemaAndLeakageGate`、`TestPromptAuditDatabasePersistsFullPromptOnEventsOnly`、`TestPromptAuditRepositoryAdmissionClaimFencingAndEventTransaction`、`TestPromptAuditRepositoryForeignKeysFiltersAndStableIdentitySnapshots`、`TestPromptAuditRepositoryHighWaterAndSafeDeletion`、`TestPromptAuditServiceConfirmationKeepsPostPreviewEventsAndConcurrentDeletesAreSafe`（均为 prompt-audit external config/Redis）、`TestEstimateOpenAIInputTokens_CompareWithOpenAIAPI`（missing external OpenAI API key）；均不命中 Task 24 migration/Live/body/auth capability。cleanup `rm -rf` JSON 为 `success=true, exit_code=0`，local tar `Test-Path=False`。双 generate 实际在 `6d4b48d6d` 执行，之后仅有 test fixture/docs/OpenSpec 变更；reviewer fix `196ee1488` 补强 OAuth test 的完整 prompt 下界及失败 cleanup，全部 `TestOpenAIImages_*` 与当前 HEAD `make test`（215 files / 1626 tests）均 exit 0，未修改生产代码，因此不重跑 remote integration。
- Task 24 fresh review closure：Task reviewer session `ses_05301c2ceffeZjH8LkLhMnmJ9L` 首轮因逐项远程证据不足返回 `CHANGES_REQUIRED`；补齐上述 evidence 并提交 OAuth test hardening `196ee1488` 后，复核为 `APPROVED`，无剩余 Critical/Important/Minor finding。Controller 接受既有 OpenSpec 7.2 勾选；Task 25/7.3 专项审查仍未执行。

- Task 25 / OpenSpec 7.3 专项审查以 `0ce1457573545383c698f9265504b230e0415a9e` 为任务起点，最终源码/test HEAD 为 `0a1234bc0`；生产 ownership chain 为 `dd58e6f6a`、`518bd5ff1`、`6b560cd1f`、`e63a6d380`、`dda0a2e72`、`23204b0db`、`0a1234bc0`，`e437a9e5b` 只是初版 doc commit。fresh review 证明 handler 预测 channel/account/OAuth/Grok 模型不具权威性，最终由 service forwarding/build 层发布真实 outbound model。本会话未提供 CodeGraph MCP，以下是精确源码、调用边和命名测试证据，不声称 CodeGraph trace。
- `RegisterGatewayRoutes` 在 `/v1/live` 与 `/backend-api/codex/realtime/calls` 前均安装 `bodyLimit -> ClientRequestID -> UsageDetailCapture -> OpsErrorLogger -> InboundEndpoint -> APIKeyAuth -> compositeTarget -> RequireGroupAssignment`。`OpenAIGatewayHandler.Live` 要求 OpenAI 分组且 `AllowLive=true`，保留 JSON/multipart SDP 的 `Session`，完成安全审计、计费和用户并发后调用 `CreateLiveCall`；因此 Live 不绕过分组授权或调度。
- Live 的并发顺序为：`CreateLiveCall` 先通过 `SelectAccountWithSchedulerForCapability(... OpenAIEndpointCapabilityLive ...)` 取得 scheduler selection，再取得 Live lease；`createUpstreamLiveCall` 返回后才调用 `selection.ReleaseFunc()`。创建失败会释放 Live lease，并按既有 OpenAI failover policy 换号；创建成功才保存绑定 API key、用户和分组的 call record。此前“上游创建前释放 scheduler selection”的表述错误，现已更正。
- 普通 OpenAI HTTP 仍采用 `ChatCompletions/Responses -> newJSONRequestBody -> ReadRaw -> SetUsageRequestBody -> SetEffectiveBytes -> BindOpenAIRequestBodyHandle -> Forward*`。Chat 向 `ForwardAsChatCompletions` 传入非空 `effectiveBody` 时，`openAIRequestBodyBytes` 直接返回该参数；只有 body 为空才回退读取绑定 handle。模型转换及 prompt-cache derive/inject 后，builder 以 token 而非已有 handle 调用 passthrough，故对 `responsesBody` 创建独立 owned `RequestBodyHandle`，`openAINewRequestWithBodyHandle` 设置 `GetBody=handle.Open`，本次上游尝试结束后清理；failover 的下一次 `ForwardAsChatCompletions` 以同一 effective body 重建新的转换 body/handle。Live 是独立 SDP/session 控制路径，不注入 Chat/Responses prompt-cache 语义。
- `requestBodyCoordinator` 在实际创建它的 ChatCompletions、Responses、Embeddings、Images 等 HTTP handler 路径入口冻结 JSON/multipart raw body；SHA 相同的 effective body 复用 raw handle，派生 effective/OAuth/multipart handle 可回放，且仅这些路径 defer 的 `Cleanup` 去重释放 handle 并删除 multipart 临时文件。`TestRequestBodyCoordinator_CleanupGinTerminationPaths` 覆盖成功、拒绝、取消、panic 和上游 4xx/5xx；`TestOpenAIGatewayHandler_ResponsesDecodesGzipRequestBody`、四个 Responses spool/read-error 测试均通过。
- 普通 Responses HTTP 的既有精确 replay 证据为 `go test ./internal/handler -run '^TestGatewayHandler_MessagesAndResponsesReplayLargeBodiesAcrossFailover$' -count=1 -v`，exit `0`；其 `responses-identity` 与 `responses-gzip` 子例覆盖 `/v1/responses` 大 body 在 failover 后的重放。该测试不验证 prompt-cache 派生；对应普通 OpenAI-compatible prompt-cache 证据为 `go test ./internal/service -run '^TestForwardAsChatCompletions_APIKeyAutoInjectsPromptCacheKey$' -count=1 -v`，exit `0`。仓库没有将两项边界合并为单一测试。
- `UsageDetailCapture` 在 handler 前创建可继续补充的 mutable collector；仅在 usage 提交前由 `BuildUsageDetailSnapshot` 冻结 snapshot。Live 的 `finalizeLiveCall` 用 `MarkLiveCallClosed` 保证只释放一次租约并写一条零 token `RequestTypeLive` usage。HTTP/WS failed usage 则在入队前快照 detail、channel 字段和 forwarding 层报告的精确上游模型。
- `OpenAIWSIngressHooks.OnOutboundRequest(turn, finalPayload, effectiveModel)` 在每个实际 `response.create` 上游写入前同步调用，覆盖 ctx_pool、HTTP bridge、passthrough 首帧及后续 turn；model-less passthrough frame 使用已写入的 `session.update` effective model。handler 只冻结 callback 的 `effectiveModel`，不再预测 account mapping 或 OAuth normalization。`TestOpenAIResponsesWebSocketFailedUsageFreezesChannelThenAccountMappedModel` 的 RED 为 persisted `gpt-client`、预期/实际 outbound `gpt-account`；passthrough 首帧 RED 为 persisted 预测 `gpt-account`、实际 outbound `gpt-channel`；session.update 后 model-less turn RED 为 stale `gpt-initial`、实际 session model `gpt-session`；修复后上述三项均 outbound==`UsageLog.UpstreamModel`，session case 还断言只有一条 failed usage。`TestNormalizeOpenAIModelForUpstream` 覆盖 OAuth `gpt-5.6 -> gpt-5.6-sol`；callback 报告 service 已完成 normalize 后的 outbound payload/model，不在 handler 重复该规则。相关 handler/service focused 命令与全包 `go test ./internal/handler`、`go test ./internal/service` 均 exit `0`。
- `638f9babe` 扩展同一 WS regression fixture 的 OAuth account/credentials，并以 ctx_pool warm connection 避开 OAuth 固定 `chatgpt.com` endpoint 后的第二个真实 OAuth request 证明：capture outbound `gpt-5.6-sol`，唯一 failed `UsageLog.UpstreamModel` 同为 `gpt-5.6-sol`。failed-usage 顺序为 `OnOutboundRequest` 冻结 final model，rate-limit `UpstreamFailoverError` 令 `AfterTurn` 跳过非-failover submit，outer terminal failover path 单次提交。初次“duplicate” RED 不是真实生产双写：`waitForOpenAIFailedUsageLog` 从 `lastLog` 返回却不消费 `created` channel，后续 uniqueness assertion 读取的是同一条 queued log；改为直接消费最终 channel 后总数为一，故未添加全局锁、时间窗口、repo 去重或生产 dedup 代码。post-fix sensitivity mutation 临时将 callback 改回 `reqModel`，`TestOpenAIResponsesWebSocketOAuthFailedUsageUsesNormalizedOutboundModel` 如预期 FAIL（expected `gpt-5.6-sol`，actual `gpt-5.6`）；恢复 callback 后 GREEN，mutation 未提交。四场景 channel->account、OAuth normalization、passthrough first-frame、session.update model-less 均 outbound==persisted，后者和 OAuth case 都断言无 duplicate。四项 focused handler command、`go test ./internal/handler -count=1`（60.006s）和 `go test ./internal/service -count=1`（103.014s）均 exit `0`。
- 第三轮 `518bd5ff1` 将 failed-usage 状态严格限定到 outbound turn：handler 在 `RewriteRequest` 和 `BeforeTurn` admission 清空 attempted/exact model，故前一 turn 已 outbound 后，下一 turn 被 rewrite/BeforeRequest 本地拒绝不会沿用 stale model；account failover 是新的 account attempt，必须重置该 state，最终由 `e63a6d380` 在真实 Responses WebSocket selection 修复。passthrough 首帧 `WriteFrame` 失败显式单次 `AfterTurn(1,nil,err)`；首轮 non-429 `error` event 结束 turn 并由该回调收口，429 仍构造既有 `UpstreamFailoverError`。HTTP bridge 将 attempted/callback 从 ingress 预测位置移入完成 prepare、Grok patch 与 request build 后、`httpUpstream.Do` 前，callback 接收最终 HTTP body/model；本地构建失败不标 attempted。`OpenAIWSIngressHooks` contract 同步覆盖 HTTP Do；WS equal model 的 `UpstreamModel` 保留路径加注释，防止回退 requested model。OAuth fixture 不再原地改动 pool account：warm 后 repository 换同 ID 新副本，pool 保持旧副本；移除 `DisableFailover` 误导选项。focused handler/service 命令、`go test ./internal/handler -count=1`（59.721s）和 `go test ./internal/service -count=1`（102.198s）均 exit `0`。
- 更正上一段第三轮测试表述：`518bd5ff1` 未包含所称四项新回归。本轮 `48ee3febc` 实际修正 OAuth fixture（warm API-key copy 与 assertion OAuth copy 均独立，`Close()` 先 `CloseOpenAIWSPool()`）并新增 `TestPassthroughLifecycle_FirstWriteFailureCallsAfterTurnOnce`（callback outbound model 与单次 `AfterTurn(1,nil,err)`）及 `TestProxyOpenAIWSHTTPBridgeTurnLocalBuildFailureDoesNotPublishOutbound`（invalid body build failure 下 callback=false、attempted=false、无 HTTP Do）。`go test ./internal/handler -count=1`（63.114s）和 `go test ./internal/service -count=1`（103.782s）均 exit `0`；其余两项 reviewer 请求的独立 handler failed-usage regression 仍未在本 commit 中声称完成。
- 最终补充：`2b6ef3f3b` 增加 `TestOpenAIResponsesWebSocketPassthroughNonRateLimitErrorRecordsFailedUsage`，server_error 首帧断言 client internal close、capture model==唯一 failed `UsageLog.UpstreamModel`、无 duplicate；临时将 non-429 branch 改回 `return nil` 后该测试 FAIL（预期 close/failed usage，实际 graceful relay），恢复后 GREEN。`3c9b0c34f` 扩展 `TestOpenAIResponsesWebSocketRejectsLaterCrossProviderCompositeRoute`：消费首 turn 成功 usage、只见首 turn upstream frame，第二 turn RewriteRequest 本地拒绝后无第二 usage/failed usage/上游 frame。临时移除两处 reset 调用未使该测试 FAIL，因为 rewrite rejection 在 `AfterTurn` 前直接返回，故此 mutation 是 post-fix inconclusive，不能称为 RED。focused command 覆盖四边界、四模型、first rate-limit/failover、turn permits、prompt cache 均 exit `0`；最终 `go test ./internal/handler -count=1` exit `0`（63.632s），`go test ./internal/service -count=1` exit `0`（104.415s）。
- migration `188` 接受 request type 5，`189` 将 `groups.allow_live` 默认为 false；非 OpenAI 分组的 service create/update 会清空 `AllowLive`。Task 24 的 `20648b826` 固定 12/12 embedded-FS、两个 `186_*`、`190_add_users_email_alias_dedup_index_notx.sql`、本地/上游同号 172 与 181、filename identity、reapply count 和 checksum；既有 archive `e98ce7a784023a400221ce71f859de1f0708ff4a` 的空库与升级 migration target 分别 PASS（4.86s、4.91s），本轮未重跑远程或 Task 24 full gate。
- Account attempt isolation：`f1ee1e518` 误改无关 Messages loop，未修真实 WS path。真实两账号 RED 为 account 1 已 outbound `gpt-account-1` 后 failover，account 2 在 callback 前真实 429 handshake 失败；预期 account 2 `gpt-account-2`，实际 leaked account 1 `gpt-account-1`。`e63a6d380` 精确恢复 Messages attempted-only reset，并在 Responses WebSocket account attempt 用 full reset；`TestOpenAIResponsesWebSocketFailoverResetsExactModelBeforeSecondAccountDialFailure` GREEN，唯一 failed log 属于 account 2，model 为 account 2。
- `dda0a2e72` 是 HTTP/relay terminal ownership 的中间修复：保留普通 HTTP/Messages attempted-only reset，并确认 WS full reset 只进入 Responses WebSocket account attempt；其普通 HTTP account-mapped model 和 relay admission ordering 后续由 `23204b0db` 收紧。
- `23204b0db` 将普通 HTTP Responses exact model 统一到 service shared context helper：handler 每次 account attempt 只清旧值、不预测模型；service 在 final request build 与 body transforms 完成后、HTTP `Do` 前发布最终 body model。真实 RED 为 `TestOpenAIGatewayHandler_ResponsesFailedUsageUsesFinalOutboundModel` 捕获 outbound `gpt-5.4`、persisted `gpt-5.4-high`；GREEN 为 actual outbound==persisted `gpt-5.4`。WS full reset不进入普通 HTTP Responses 或 Messages。
- relay terminal 顺序最终为：terminal downstream `WriteFrame` 成功后，先 `OnTurnComplete`/adapter `AfterTurn` 提交 turn 1 success 并释放 permit，再由 `AfterClientWrite` 开放 follow-up admission；写失败不发布 success。`TestRelayCommitsTerminalBeforeFollowupAdmission` 的 race RED 为 actual turn `1`、expected `2`，修复后 GREEN；`TestRelayOnTurnCompleteWaitsForTerminalDownstreamWrite`、`TestRelayBeforeWriteUpstreamWaitsForTurnPermit`、relay metrics 与首写失败 focused 均 GREEN。
- `0a1234bc0` 补齐 intentional drain 与 Grok final model。Drain RED：turn 1 已完成、turn 2 已发，client 断开后 upstream terminal usage 被 drain，但 adapter 仅见 `turns=1`、漏掉 turn 2 `AfterTurn`；现在仅 `dropDownstreamWrites` intentional drain 的 matching terminal 调用既有 completion callback并释放 active turn 一次，以 graceful `drain_terminal` 退出，不伪造 downstream write、`AfterClientWrite` 或 duplicate。relay internal `TestRunUpstreamToClient_ErrorAndDropPaths/drop downstream and stop on terminal` 与 adapter `TestPassthroughLifecycle_ClientDisconnectDrainsSecondTurnCompletionOnce` GREEN。
- Grok RED：ordinary `/v1/responses` 实际 outbound `grok-4.3`，persisted `grok-account-routing`。`buildGrokResponsesRequest` 将 final body model 附到成功构建的 request，`forwardGrokResponses` 在 `httpUpstream.Do` 前发布；`TestOpenAIGatewayHandler_GrokResponsesFailedUsageUsesFinalOutboundModel` GREEN，actual outbound==persisted `grok-4.3`。该 test 与 drain lifecycle test 的 focused `-count=10` 均通过；terminal write/race、四模型、account reset 既有矩阵保持 GREEN。
