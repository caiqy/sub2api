...
- remote 首次解包命令因 ssh-skill Windows native fallback 在本机提前展开 `$d`，以 `/source.tar` 失败，未执行测试；改为绝对路径命令后完整 gate 通过。完整远端日志有 `12` 条 SKIP、`0` 条 FAIL，SKIP 与首次命令插值失败均保留为 concern。archive 已删除、远端唯一目录经 `test ! -e` 核验不存在；日志保留在 `C:/Users/caiqy/AppData/Local/Temp/opencode/sub2api-v0.1.163-368822c706f84dee88147b896e6bed00-integration.log`。
