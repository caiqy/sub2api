...
- Task 25 / OpenSpec 7.3 专项审查以 `0ce1457573545383c698f9265504b230e0415a9e` 为任务起点，最终审查源码 HEAD 为 `be9b9023b`；`e437a9e5b` 只是初版 doc commit。fresh review 发现 WS failed-usage 模型快照缺口后由 `be9b9023b` 修复。本会话未提供 CodeGraph MCP，以下是精确源码、调用边和命名测试证据，不声称 CodeGraph trace。
...
- `UsageDetailCapture` 在 handler 前生成不可变 detail snapshot。Live 的 `finalizeLiveCall` 用 `MarkLiveCallClosed` 保证只释放一次租约并写一条零 token `RequestTypeLive` usage。HTTP/WS failed usage 则在入队前快照 detail、channel 字段和精确上游模型；`be9b9023b` 使 WS 首帧先按 channel mapping、再按 account mapping 冻结模型，后续帧在 account mapping 后冻结，避免异步 failed usage 回退到 client model。`go test ./internal/service -run '^TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughHeadersUsePromptCacheAndTurnState$' -count=1 -v` 与 `go test ./internal/handler -run '^TestOpenAIResponsesWebSocketFailedUsageFreezesChannelThenAccountMappedModel$' -count=1 -v` 均 exit `0`；后者的 RED 为期望 `gpt-account`、实际 `gpt-client`，修复后 GREEN。
...
- 结论：Task 25 证据为 `protected`，WS failed-usage 映射缺口已由 `be9b9023b` 和确定性回归覆盖；`VERSION` 仍为 `0.1.159.6`。OpenSpec 7.3 不勾选，Task 26、final verify、release、tag、push、deploy、远程操作仍不在范围内。
