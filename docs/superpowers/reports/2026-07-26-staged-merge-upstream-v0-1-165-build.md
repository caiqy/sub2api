...
| changed-files 与本地能力交集 | `v0.1.162` | 已闭合 | Task 16 独立复核：190 个 changed files 中，4 个指定能力行覆盖 38 个精确交集路径（`8+9+8+13`），`protected=4`、`manual=0`、`gap=0`；见 v0.1.162 Task 16 closure。 |
...
- 审查起点为 `11889c61c`。当前 agent 未暴露 CodeGraph 工具，改用精确 Git range、知识库能力入口和源码定义/调用点读取；未读取全量 plan，未执行远程操作、Task 17 或任何源码修复。TDD N/A：本任务只补充文档审查证据，不改变行为。
- 计数可复核：`git diff --name-only 'v0.1.161^{}..v0.1.162^{}'` 为 `190` 条；从该集合按下列四项能力列出、再交集核对的路径为 `38` 条，互斥分组为 settings/runtime `8`、trusted proxy/client-IP `9`、Grok cache/sticky `8`、S3/image `13`。矩阵行数 `4`，每行均有自动测试与当前源码入口，故 `protected=4`、`manual=0`、`gap=0`；没有把不在这 38 条中的泛化回归计入该数字。

| 能力 | 上游触及与精确交集 | 实际入口/调用链 | 自动或人工证据 | 结论 |
| --- | --- | --- | --- | --- |
| settings JSON migration/backfill、解析默认值、启动与热更新 | 8 条：`config.go`、`config_test.go`、`env_reachability_test.go`、`cmd/server/main.go`、`setting_service.go`、`setting_parse.go`、`setting_update.go`、`setting_service_update_test.go` | `main.runMainServer -> config.LoadForBootstrap` 解析 env/default 并发布初始 snapshot；`ProvideSettingService -> LoadForwardedClientIPSettings` 读取三键、缺 header JSON 或 mode-v2 时 `SetMultiple` 回填、再 `SetForwardedClientIPSettings`；`UpdateSettings -> refreshCachedSettings -> SetGatewayControlRuntime/SetForwardedClientIPSettings` 同步重建 scheduler/sticky 与 IP runtime snapshot。 | 本任务聚焦 GREEN：config 的 header snapshot、trusted proxy、image-storage env；service 的 forwarded migration/backfill/read-failure、settings refresh。Task 14 仅 merge/cleanup 的限定编译证据；Task 15 final-HEAD full gate 及 integration 的回归证据另见下。 | closed；不存在“只写 DB 不生效”的路径。 |
...
| Grok client-tool cache 与 session/platform Sticky | 8 条：`openai_gateway_grok.go`、`openai_gateway_grok_cache.go`、`openai_gateway_grok_cache_test.go`、`openai_gateway_grok_chat_bridge.go`、`openai_gateway_grok_chat_bridge_test.go`、`openai_gateway_messages.go`、`openai_ws_http_bridge.go`、`openai_ws_http_bridge_test.go` | HTTP `forwardGrokResponses -> resolveGrokCacheIdentity -> applyGrokResponsesCacheIdentity -> applyGrokFreeRequestToolCacheRoute`；WS bridge 走同一 identity/tool-route 后构建 Grok request。identity 由 API key、mapped model、stable session/content seed 隔离；runtime platform Sticky 由 settings snapshot 被 `OpenAIGatewayService`/scheduler 读取。 | 本任务聚焦 GREEN：stable append-only identity、known-free client-tool cache default、request opt-in override。Task 15 覆盖最终 HEAD 的 cache 提交、HTTP/WS bridge/terminal 及 failover 回归；Task 14 只确认冲突融合时没有丢失 sticky/owner/spool/redaction/usage 契约。 | closed；HTTP 与 WS 采用同一 tenant-isolated cache identity，且保留 platform sticky。 |
| S3 backup / image storage / async image path | 13 条：`image_storage_env_test.go`、`backup_service.go`、`backup_service_test.go`、`image_storage_settings.go`、`image_storage_settings_test.go`、`image_task.go`、`admin/backup_handler.go`、`image_task_handler.go`、`routes/admin.go`、`routes/gateway.go`、`routes/gateway_test.go`、`service/wire.go`、`cmd/server/wire_gen.go` | Wire `ProvideBackupService.Start` 启动备份调度；`ProvideImageStorageSettingService` 将 config fallback 与 S3 factory 绑定；`ProvideImageTaskService -> Resolver -> resolve` 缓存有效 uploader，`Update -> Invalidate` 后下次请求重建；`AsyncImageHandler -> ImageTaskService.Complete -> ImageResultUploader.Rewrite` 再落 Redis。admin S3/image-storage 与 gateway async routes 均已注册。 | 本任务聚焦 GREEN：env active、image setting 无重启 toggle/reuse backup credential、task offload。源码确认未配置/转存失败不会把 base64 回落到 Redis，并保留 owner/group/moderation/audit gate。Task 15 full regression/integration 是最终门禁；Task 14 仅审查 Wire 融合与路由/权限保留。 | closed；备份与图片共用凭证时仍经加密/运行时 resolver，异步路径可即时重载。 |

- Task 14 与 Task 15 的证据边界：Task 14 merge `8bda73544d6e26a323f101e5c68981634f0375ab`（及 cleanup `46eb292c04f14630dcfb31b28f6e83f541029d93`）只证明冲突融合、无 unmerged/marker、限定 Go 编译与 vue-tsc，明确未跑 full gate。Task 15 final archive `8807d9a34` 是当前起点祖先，且到 `11889c61c` 之后没有源码 diff；它才证明 `make test`（204 files/1549 cases）、build、双 generate/Ent-Wire zero diff、静态检查和完整 integration 全绿。Task 16 的上列源码/聚焦测试独立补足 capability-level closure，不把 Task 14 静态结论误写为运行时验证。
- Task 16 额外验证：聚焦 config、unit-tag server/middleware、service 测试均 exit `0`；`git ls-files -u` 为 0、精确 tracked conflict marker 为 0、`git diff --check` exit `0`、`backend/cmd/server/VERSION` 精确为 `0.1.159.6`。结论：`DONE`，无 capability gap 或本任务 concerns。
