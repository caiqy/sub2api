...
- Task 25 / OpenSpec 7.3 independent capability review (start/HEAD `0ce1457573545383c698f9265504b230e0415a9e`): CodeGraph MCP was not exposed to this reviewer, so this is explicitly a source/function/call-edge review, not a CodeGraph trace. `RegisterGatewayRoutes` applies `bodyLimit -> ClientRequestID -> UsageDetailCapture -> OpsErrorLogger -> InboundEndpoint -> APIKeyAuth -> compositeTarget -> RequireGroupAssignment` before both `/v1/live` and `/backend-api/codex/realtime/calls` (`backend/internal/server/routes/gateway.go:160-188,288-297`). `OpenAIGatewayHandler.Live` then requires an OpenAI group with `AllowLive`, validates JSON or multipart SDP/session without changing `Session`, runs security audit/billing/user concurrency, and calls `OpenAIGatewayService.CreateLiveCall` (`backend/internal/handler/openai_live.go:21-110,112-165,231-236`). `CreateLiveCall` selects through the existing `SelectAccountWithSchedulerForCapability(... OpenAIEndpointCapabilityLive ...)`, acquires a Live lease, releases the scheduler selection before upstream creation, applies existing failover policy, and stores the identity-bound record (`backend/internal/service/openai_live.go:120-238`). Thus Live does not bypass the scheduler or group authorization; it is intentionally a separate SDP/session control path rather than an HTTP/Responses prompt-body path.
- Prompt cache and body ownership boundary: normal OpenAI HTTP remains on `ChatCompletions/Responses -> newJSONRequestBody -> ReadRaw -> SetUsageRequestBody -> SetEffectiveBytes -> BindOpenAIRequestBodyHandle -> Forward*` (`backend/internal/handler/openai_chat_completions.go:23-146`; `backend/internal/handler/openai_gateway_handler.go:243-327`). `ForwardAsChatCompletions` first reads that bound handle, then derives/injects the compatibility prompt-cache key and builds its upstream request from a replayable body handle (`backend/internal/service/openai_gateway_chat_completions.go:53-64,109-125,202-239,259-264`; `backend/internal/service/openai_gateway_request_body.go:61-75,100-121`). Live has no Responses/Chat prompt cache semantic to inject and does not modify that route.
- JSON/multipart/raw/effective/OAuth body lifecycle is protected by `requestBodyCoordinator`: decoded JSON and multipart raw handles freeze at ingress, SHA-equal effective bodies reuse raw, derived effective/OAuth/multipart handles are separately replayable, and `Cleanup` deduplicates raw/effective/OAuth handles plus removes form files (`backend/internal/handler/request_body_coordinator.go:32-102,112-217,249-279`; `backend/internal/service/request_body_handle.go:93-185`). Existing handler defers cleanup on all normal OpenAI HTTP entry points; `TestRequestBodyCoordinator_CleanupGinTerminationPaths` covers success, rejection, cancellation, panic, and upstream 4xx/5xx. `TestOpenAIGatewayHandler_ChatAndEmbeddingsReplayMappedSpoolAcrossFailover` covers mapped raw/compressed spools across failover.
- Usage/detail/session boundary: `UsageDetailCapture` is installed before the gateway handlers and builds an immutable normalized snapshot (`backend/internal/server/middleware/usage_detail_capture.go:58-118`). HTTP/WS failed usage snapshots the current detail, exact upstream model, channel fields, protocol type, and response before queuing (`backend/internal/handler/openai_gateway_handler.go:2504-2621`); WS success snapshots channel fields and `ExtractClientSessionID` before async record submission (`backend/internal/handler/openai_gateway_handler.go:2186-2225`). Live calls have no per-turn token/detail payload: terminal `finalizeLiveCall` atomically gates on `MarkLiveCallClosed`, releases the lease once, and creates exactly one zero-token `RequestTypeLive` usage record keyed by the call hash (`backend/internal/service/openai_live.go:743-791`; `backend/internal/service/openai_live_lifecycle_test.go:255-298`). This is a distinct lifecycle record, not a missing HTTP/WS failed-usage path.
- Capability/config/frontend boundary: migration `188` admits request type 5 and `189` defaults `groups.allow_live=false`; service create/update clears `AllowLive` outside OpenAI (`backend/migrations/188_allow_live_usage_request_type.sql:1-6`, `backend/migrations/189_add_group_allow_live.sql:1`, `backend/internal/service/admin_group.go:480-523,798-840`). Frontend exposes the switch only for OpenAI groups, asks the capability endpoint before enabling, and treats `live` as a first-class request type without mapping it to legacy stream fields (`frontend/src/views/admin/GroupsView.vue:1443-1474,5278-5314`; `frontend/src/utils/usageRequestType.ts:9-33`).
- Focused review commands all exited `0`: handler Live/body/failed-usage matrix (`11.628s`); service Live lifecycle/prompt-cache matrix (`5.415s`); repository request-type matrix (`2.803s`) and Live cache/lease matrix (`1.992s`); unit-tag group platform sanitization (`1.865s`); frontend `GroupsView.duplicate` plus `UsageFilters` Vitest (`2 files/11 tests`) and `vue-tsc --noEmit`. The initial non-unit group command matched zero tests and is not counted; the `-tags=unit` rerun is the evidence.
- Task 24 migration connection, not rerun: `20648b826` adds the final embedded-FS `12/12` assertion and explicit two-`186_*`/`190_*_notx.sql` checks (`backend/internal/repository/migrations_schema_integration_test.go:209-320`). Its filtered-FS then full-FS replay verifies local/upstream same-number `172` and `181`, two upstream `186`, full filename identity, reapply record count, and checksums. The prior remote archive `e98ce7a784023a400221ce71f859de1f0708ff4a` ran both migration targets PASS (`4.86s` empty schema; `4.91s` upgrade); the successful log does not print `12/12`, which is correctly evidenced by the fixed assertion plus target PASS. `190_add_users_email_alias_dedup_index_notx.sql` remains non-transactional and the runner sorts and records full filename/checksum before applying `*_notx.sql` statements (`backend/internal/repository/migrations_runner.go:160-240`; migration file lines 1-8). No Task 24 full local/remote gate was repeated.
- Conclusion: `protected`, no capability `gap` or production change. This closes Task 25 evidence only; `VERSION` remains `0.1.159.6`, OpenSpec 7.3 is deliberately not checked, and Task 26/final verification, release, tag, push, deploy, and remote operations remain out of scope.
