...
- V2 生命周期：后续 turn 的 hooks、并发槽和 metadata 切换受单 active turn permit 保护；阻塞 reader 可被新 deadline/control 唤醒。
- 流间隔超时：仅在首业务事件后启动，并补齐 OpenAI OAuth passthrough 与 Responses→Chat fallback。
- 失败审计：drain 终态保留已知 token 的全部维度，未知消耗明确标记 `usage_state=unknown`，且保持零成本。
...
- `go test ./internal/config ./internal/pkg/openai ./internal/handler/admin ./internal/handler ./internal/service ./internal/service/openai_ws_v2 -count=1`：PASS。
- `go test -tags unit ./internal/service -run 'TestWriteFailedUsageLogBestEffort|WSPassthroughUsageMeta' -count=1`：PASS。
...
- 三轮聚焦修复审查后，Thorough reviewer 最终结论：ready，无 Critical/Important。
