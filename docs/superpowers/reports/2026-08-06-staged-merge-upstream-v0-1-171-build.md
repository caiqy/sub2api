...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | advanced/layered scheduler、pool mode、DB recheck、WaitPlan fallback、同账号重试 | `OpenAIGatewayHandler` -> `OpenAIGatewayService` -> `OpenAIAccountScheduler.Select` -> sticky/session -> layered filter -> slot/`WaitPlan` | `openai_account_scheduler*.go`、`openai_gateway_scheduling.go`、`gateway_handler_sticky_fallback_test.go` | `v0.1.170` | 人工审查：候选过滤、粘性绑定、槽位释放后 DB recheck 与重新选号 | `gap` | plan L243；本节两段 changed-files；CodeGraph Task 3 scheduler entry |
| 2 | Grok/platform/session/previous-response sticky、privacy、image capability | scheduler/WS 入口 -> platform 归一化与 sticky lookup -> capability/privacy/transport 过滤 | `openai_account_scheduler_layered.go`、`openai_gateway_grok.go`、`openai_ws_*` | `v0.1.170`、`v0.1.171` | 人工审查：跨轮绑定、privacy 和 image 条件不会被上游选择短路 | `gap` | plan L244；本节两段 changed-files；CodeGraph Task 3 scheduler entry |
| 3 | OpenAI HTTP/WS、turn ownership、最终 outbound model、failed usage、prompt-cache reuse、proxy circuit | HTTP `OpenAIGatewayHandler` 或 WS forwarder -> `OpenAIGatewayService` -> forward/usage billing -> usage worker | `openai_gateway_handler.go`、`openai_ws_*`、`openai_gateway_usage.go`、`gateway_usage_billing.go` | `v0.1.170`、`v0.1.171` | 人工审查：流式失败、WS 终止和切号均不重复计费 | `gap` | plan L245；本节两段 changed-files；CodeGraph Task 3 gateway/usage entry |
| 4 | alpha-search、Responses fallback、PAT 401 副作用、WebSearchCalls、body handle | alpha-search handler/service -> upstream request builder -> matched `RequestBodyHandle` -> fallback/retry | `openai_alpha_search.go`、`openai_gateway_request_body.go`、`openai_gateway_handler.go` | `v0.1.170`、`v0.1.171` | 人工审查：fallback/retry 后 handle 可重放且最终释放 | `gap` | plan L246；本节两段 changed-files；CodeGraph `buildOpenAIAlphaSearchRequest` |
| 5 | 请求体 spooling/replay/cleanup、异步图片、对象存储、图片输入计费 | request-body handle -> Images handler/service -> storage/usage billing | `openai_gateway_request_body.go`、`openai_images.go`、`image_storage.go` | `v0.1.170` | 人工审查：body 不泄漏、不提前关闭，图片计费仅一次 | `gap` | plan L247；`v0.1.170` changed-files；CodeGraph Images entry |
| 6 | 统一 prompt/security audit 与 Images | gateway/images handler -> prompt snapshot/coordinator -> legacy moderation -> Images forward | `securityaudit/prompt_*.go`、`content_moderation.go`、`openai_images.go` | `v0.1.170`、`v0.1.171` | 人工审查：每请求最多一次 moderation/payload freeze；关闭态不构造大 payload；`ReleaseText` 前仍可审核 | `gap` | plan L248；本节两段 changed-files；CodeGraph `OpenAIImagesRequest.ModerationBody` |
| 7 | settings 热更新、repository scoped update、API Key auth cache、session binding/step-up | setting handler -> `SettingService` -> repository scoped update/callback；auth middleware -> `APIKeyService.GetByKey` -> L1/L2 auth cache | `setting_handler*.go`、`setting_update.go`、`api_key_repo.go`、`api_key_auth_cache*.go` | `v0.1.170`、`v0.1.171` | 人工审查：部分更新不清空本地字段，失效事件和 session/step-up 不遗漏 | `gap` | plan L249；本节两段 changed-files；CodeGraph APIKey auth-cache entry |
| 8 | subscription quota reset、续期、退款/余额、receipt、tombstone、outbox | payment/refund handler -> `SubscriptionService` -> `UserSubscriptionRepository` -> transaction/receipt -> cache invalidation outbox | `subscription_service.go`、`payment_refund.go`、`user_subscription_repo.go`、`backend/migrations/*.sql` | `v0.1.170`、`v0.1.171` | 人工审查：单窗口资格、事务锁、提交后 invalidation 与失败回滚 | `gap` | plan L250；本节两段 changed-files；CodeGraph `AdvanceQuotaCycleWithReceipt` |
| 9 | 用户资源控制、分组复制/批量限额、account shadow、前端本地定制 | admin handler/service -> account/group repository；`AccountsView`/`GroupsView`/Settings/Usage/Subscription/Channels/mobile UI -> admin APIs | `admin/account_handler.go`、`group_repo.go`、`AccountsView.vue`、`GroupsView.vue`、`frontend/src/api/admin/*.ts` | `v0.1.170`、`v0.1.171` | 人工审查：变更页面保持本地入口和资源控制语义 | `gap` | plan L251；本节两段 changed-files；CodeGraph GroupRepository/frontend references |
| 10 | Codex identity、动态版本、UA、过载重试 | OpenAI HTTP/WS/alpha-search entry -> codex identity/version resolver -> upstream request/retry -> final account/model/error | `openai_codex_identity.go`、`openai_codex_version_sync_service.go`、`pkg/openai/request.go`、`openai_alpha_search.go` | `v0.1.171` | 人工审查：模型列表、账号测试和 alpha-search 共用身份来源，重试保留最终语义 | `gap` | plan L252；`v0.1.171` changed-files；CodeGraph Codex/gateway entry |
| 11 | Ent/Wire、Go/pnpm 依赖、CSP/deploy 配置、migrations | schema/provider/manifest source -> `make -C backend generate` -> Ent/Wire output；migration runner -> ordered filename/checksum；frontend manifest -> pnpm lock | `backend/ent/schema/`、`backend/**/wire.go`、`backend/go.mod`、`backend/go.sum`、`deploy/config.example.yaml`、`backend/migrations/*.sql` | `v0.1.170`、`v0.1.171` | 人工审查：source-driven generation、两轮无 diff、完整 migration filename/排序/checksum 和本机 integration | `gap` | plan L253；本节两段 changed-files；plan L41-L45 |
