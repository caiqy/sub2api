...
// The UserResourceOverrideFunc type is an adapter to allow the use of ordinary function as a Querier.
type UserResourceOverrideFunc func(context.Context, *ent.UserResourceOverrideQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UserResourceOverrideFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UserResourceOverrideQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UserResourceOverrideQuery", q)
}

// The TraverseUserResourceOverride type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUserResourceOverride func(context.Context, *ent.UserResourceOverrideQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUserResourceOverride) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUserResourceOverride) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserResourceOverrideQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UserResourceOverrideQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UserResourceOverrideQuery:
		return &query[*ent.UserResourceOverrideQuery, predicate.UserResourceOverride, userresourceoverride.OrderOption]{typ: ent.TypeUserResourceOverride, tq: q}, nil
