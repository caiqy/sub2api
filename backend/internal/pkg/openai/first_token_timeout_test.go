...
func TestResponsesFirstTokenEventClassification(t *testing.T) {
	...
	require.False(t, ResponsesEventEndsFirstTokenWait([]byte(`{"type":"response.queued"}`)))
	require.False(t, ResponsesEventEndsFirstTokenWait([]byte(`{"type":"session.updated"}`)))
	require.False(t, ResponsesEventEndsFirstTokenWait([]byte(`{"type":"rate_limits.updated"}`)))
	require.False(t, ResponsesEventEndsFirstTokenWait([]byte(`{"type":"vendor.control"}`)))
	...
	require.True(t, ResponsesEventEndsFirstTokenWait([]byte(`{"type":"response.completed"}`)))
	require.True(t, ResponsesEventRecordsFirstToken([]byte(`{"type":"response.output_text.delta","delta":"x"}`)))
	require.False(t, ResponsesEventRecordsFirstToken([]byte(`{"type":"session.updated"}`)))
	require.False(t, ResponsesEventRecordsFirstToken([]byte(`{"type":"vendor.control"}`)))
