...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		result, err := func() (*service.OpenAIForwardResult, error) {
			defer releaseAccount()
			if endpoint.IsSeedance() {
				return h.gatewayService.ForwardSeedance(requestCtx, c, account, endpoint, requestID, seedanceBody)
			}
		...
		if endpoint == service.SeedanceEndpointStatus {
			if billResult := prepareSeedanceCompletionBilling(requestCtx, h, apiKey, subject, requestID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, billResult, billResult.Model, string(body), requestID)
			}
			...
			if billResult := prepareGrokVideoCompletionBilling(requestCtx, h, reqLog, apiKey, subject, taskID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, billResult, billResult.Model, requestPayloadHash, taskID)
			}
...
func grokMediaRequestBodyPreviewWithSize(contentType string, body []byte, requestInfo service.GrokMediaRequestInfo, size int64) string {
	...
	prompt := truncateString(requestInfo.Prompt, multipartMetadataPromptPreviewLimitBytes)
