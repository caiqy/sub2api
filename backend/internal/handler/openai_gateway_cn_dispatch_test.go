...
func TestAllowOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	require.True(t, allowOpenAICompatibleMessagesDispatch(context.Background(), nil), "无 key 保持放行")
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformGrok} {
		...
		require.True(t, allowOpenAICompatibleMessagesDispatch(context.Background(), apiKey),
	...
	require.False(t, allowOpenAICompatibleMessagesDispatch(context.Background(), openaiOff))
	openaiOn := &service.APIKey{Group: &service.Group{Platform: service.PlatformOpenAI, AllowMessagesDispatch: true}}
	require.True(t, allowOpenAICompatibleMessagesDispatch(context.Background(), openaiOn))
}
