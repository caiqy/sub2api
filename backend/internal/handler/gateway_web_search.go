...
func (h *GatewayHandler) WebSearch(c *gin.Context) {
	...
	for attempt := 0; attempt < 4; attempt++ {
		selected, selectErr := h.gatewayService.SelectAccountWithLoadAwareness(
			c.Request.Context(), groupID, sessionHash, searchModel, failedAccounts, "", 0,
		)
		if selectErr != nil {
			if attempt == 0 {
		...
		if isXSearch {
			nativeResp, providerName, upstreamModel, err = h.doGrokNativeXSearch(c.Request.Context(), c, account, req, searchModel, maxResults)
		} else {
			nativeResp, providerName, upstreamModel, err = h.doGrokNativeWebSearch(c.Request.Context(), c, account, req.Query, maxResults, searchModel)
		}
		if err == nil {
			break
		}
...
func (h *GatewayHandler) doGrokNativeWebSearch(ctx context.Context, c *gin.Context, account *service.Account, query string, maxResults int, model string) (*websearch.SearchResponse, string, string, error) {
	maxResults = normalizeGrokWebSearchMaxResults(maxResults)
...
func (h *GatewayHandler) doGrokNativeXSearch(ctx context.Context, c *gin.Context, account *service.Account, req grokStandaloneSearchRequest, model string, maxResults int) (*websearch.SearchResponse, string, string, error) {
	maxResults = normalizeGrokWebSearchMaxResults(maxResults)
