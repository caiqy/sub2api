...
func TestProxyHandlerUpdatePreservesCredentialFieldPresence(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "omitted", body: `{"status":"inactive"}`},
		{name: "null clears", body: `{"username":null,"password":null}`, set: true},
		{name: "values", body: `{"username":" user ","password":" pass "}`, set: true, username: "user", password: "pass"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			svc := &proxyPartialUpdateService{}
			router := gin.New()
			...
			require.Equal(t, http.StatusOK, w.Code, w.Body.String())
			require.Equal(t, tc.set, svc.input.UsernameSet)
			require.Equal(t, tc.set, svc.input.PasswordSet)
			require.Equal(t, tc.username, svc.input.Username)
			require.Equal(t, tc.password, svc.input.Password)
		})
	}
}
