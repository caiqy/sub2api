...
func (h *OpenAIGatewayHandler) GrokRealtime(c *gin.Context) {
	...
	for attempts := 0; attempts < 4; attempts++ {
		...
		candidate, _, selectErr := h.gatewayService.SelectAccountWithSchedulerForCapability(
			c.Request.Context(), apiKey.GroupID, "", sessionHash, "", failed,
		...
		release, slotStatus = h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, candidate, false, &streamStarted, reqLog)
	...
	started := time.Now()
	audioObserved, proxyErr := h.gatewayService.ProxyGrokRealtimeConn(c.Request.Context(), c, conn, upstream, eventGuard)
	upstreamModel := selection.Account.GetMappedModel(model)
