...
func TestOpenAIResponsesWebSocket_PassthroughTerminalFailureUsesCurrentTurnReasoningMetadata(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"gpt-5.4-max","stream":false,"reasoning":{"effort":"high"}}`,
	...
	require.Len(t, got.logs, 2)
	failedLog := got.logs[1]
	require.NotNil(t, failedLog.ReasoningEffort)
	require.Equal(t, "xhigh", *failedLog.ReasoningEffort)
	require.Nil(t, failedLog.RequestedReasoningEffort)
}

func TestOpenAIResponsesWebSocket_ExhaustedFailoverPreservesRequestedReasoningEffort(t *testing.T) {
	got := runOpenAIResponsesWebSocketUsageLogCase(t, openAIResponsesWSUsageLogCase{
		firstPayload:  `{"type":"response.create","model":"gpt-5.4-max","stream":false,"reasoning":{"effort":"high"}}`,
		secondPayload: `{"type":"response.create"}`,
		failureTurn:   2,
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	upstreamServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for turn := 1; turn <= turnCount; turn++ {
			...
			if tc.failureTurn == turn {
				failureEvent := ""
				switch tc.failureMode {
				case "terminal":
					failureEvent = fmt.Sprintf(
						`{"type":"response.created","response":{"id":%q,"model":%q}}`,
						responseID,
						gjson.GetBytes(payload, "model").String(),
					)
				case "exhausted":
					failureEvent = `{"type":"error","error":{"code":"rate_limit_exceeded","type":"usage_limit_reached","message":"The usage limit has been reached"}}`
				}
				writeCtx, cancelWrite := context.WithTimeout(r.Context(), 3*time.Second)
				writeErr := conn.Write(writeCtx, coderws.MessageText, []byte(failureEvent))
				cancelWrite()
				if writeErr != nil {
					upstreamErrCh <- writeErr
					return
				}
				if tc.failureMode == "terminal" {
					failureEvent = fmt.Sprintf(
						`{"type":"response.failed","response":{"id":%q,"model":%q,"status":"failed","error":{"code":"server_error","message":"terminal failure"},"usage":{"input_tokens":2,"output_tokens":1}}}`,
						responseID,
						gjson.GetBytes(payload, "model").String(),
					)
					writeCtx, cancelWrite = context.WithTimeout(r.Context(), 3*time.Second)
	...
	readTurnEvents := func(turn int) {
		if tc.failureTurn == turn && tc.failureMode == "exhausted" {
			readCtx, cancelRead := context.WithTimeout(context.Background(), 3*time.Second)
		...
		responseID := fmt.Sprintf("resp_usage_e2e_%d", turn)
		expectedTypes := []string{"response.created", "response.completed"}
		if tc.failureTurn == turn && tc.failureMode == "terminal" {
			expectedTypes = []string{"response.created", "response.failed"}
		}
...
func TestOpenAIResponses_FunctionCallOutputValidationDoesNotRetainParsedBody(t *testing.T) {
	gin.SetMode(gin.TestMode)

	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/openai/v1/responses", strings.NewReader(
		`{"model":"gpt-5.1","stream":false,"input":[{"type":"function_call_output","call_id":"call_1","output":"{}"}]}`,
	))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	require.Equal(t, http.StatusBadRequest, w.Code)
	require.Contains(t, w.Body.String(), "Responses WebSocket v2")
	_, retained := c.Get("openai_parsed_request_body")
...
func newOpenAIWSRegressionEnv(t *testing.T, cache *concurrencyCacheMock, opts openAIWSRegressionEnvOptions) *openAIWSRegressionEnv {
	...
	var upstreamMessages chan []byte
	if opts.CaptureUpstreamMessages {
		upstreamMessages = make(chan []byte, 4)
	}
		...
		upstreamServer = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			for {
				...
				if upstreamErrorsThisTurn {
					errorCode := opts.UpstreamErrorCode
					if errorCode == "" {
						errorCode = "rate_limit_error"
					}
					response = []byte(fmt.Sprintf(`{"type":"error","error":{"code":%q,"type":%q,"message":"upstream failed"}}`, errorCode, errorCode))
				}
...
func TestOpenAIResponsesWebSocketResolvesCompositeExplicitAliasOnEveryFrame(t *testing.T) {
	resolver := service.NewCompositeRouteResolver(openAIWSCompositeRouteRepo{routes: []service.CompositeModelRoute{{
		GroupID:        2,
		PublicModel:    "public-alias",
		MatchType:      service.CompositeRouteMatchExact,
	...
	for turn := 0; turn < 2; turn++ {
		select {
		case payload := <-env.upstreamMessages:
			require.Equal(t, "gpt-5", gjson.GetBytes(payload, "model").String(), "turn=%d", turn+1)
		case <-time.After(5 * time.Second):
...
func TestOpenAIResponsesWebSocketPassthroughNonRateLimitErrorRecordsFailedUsage(t *testing.T) {
	cache := &concurrencyCacheMock{
		acquireUserSlotFn:      func(context.Context, int64, int, string) (bool, error) { return true, nil },
		acquireUserGroupSlotFn: func(context.Context, int64, int64, int, string) (bool, error) { return true, nil },
		acquireAccountSlotFn:   func(context.Context, int64, int, string) (bool, error) { return true, nil },
...
func TestOpenAIGatewayHandler_ResponsesRequiresChatCompletionsCapability(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	embeddingsOnly := &service.Account{
		ID:          11,
		Name:        "openai-embeddings-only",
		Platform:    service.PlatformOpenAI,
		Type:        service.AccountTypeAPIKey,
		Status:      service.StatusActive,
		Schedulable: true,
		Concurrency: 1,
		Priority:    0,
		Credentials: map[string]any{"api_key": "sk-embeddings", "openai_capabilities": []any{"embeddings"}},
	}
	chatCapable := &service.Account{
		ID:          12,
		...
		Concurrency: 1,
		Priority:    10,
		Credentials: map[string]any{"api_key": "sk-chat", "openai_capabilities": []any{"chat_completions"}},
	}
	accountRepo := &openAIRetryAccountRepoStub{accounts: []*service.Account{embeddingsOnly, chatCapable}}
	usageRepo := &openAIChatCompletionsUsageLogRepoStub{}
	httpUpstream := &openAIRetryTrackingHTTPUpstreamStub{responses: []*http.Response{{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body: io.NopCloser(strings.NewReader(
			`{"id":"resp_123","object":"response","status":"completed","model":"gpt-5.1","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}`,
		)),
	}}}
	concurrencyService := service.NewConcurrencyService(openAIChatCompletionsConcurrencyCacheStub{})
	billingCacheService := service.NewBillingCacheService(nil, nil, nil, nil, nil, nil, cfg, nil)
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"gpt-5.1","input":[{"role":"user","content":[{"type":"input_text","text":"hello"}]}],"stream":false}`))
	req.Header.Set("Content-Type", "application/json")
	router.ServeHTTP(rec, req)
...
func TestOpenAIGatewayHandler_ResponsesDecodesGzipRequestBody(t *testing.T) {
	gin.SetMode(gin.TestMode)

	router, usageRepo, httpUpstream, cleanup := newOpenAIResponsesRequestBodyTestRouter(t)
	defer cleanup()

	reqBody := `{"model":"gpt-5.4","stream":false,"input":"hello gzip"}`
	...
	require.Equal(t, "hello gzip", gjson.GetBytes(upstreamBody, "input").String())
	require.NotNil(t, usageRepo.lastLog)
	require.NotNil(t, usageRepo.lastLog.DetailSnapshot)
	requireRequestPreviewSnapshot(t, usageRepo.lastLog.DetailSnapshot.RequestBody, reqBody)
}

func TestOpenAIGatewayHandler_ResponsesSpoolCreateFailureReturns503(t *testing.T) {
	gin.SetMode(gin.TestMode)

	router, _, httpUpstream, cleanup := newOpenAIResponsesRequestBodyTestRouter(t)
	defer cleanup()
	...
	require.Empty(t, httpUpstream.requests)
}

func TestOpenAIGatewayHandler_ResponsesSpoolTransportFailureReturns503(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router, usageRepo, httpUpstream, cleanup := newOpenAIResponsesRequestBodyTestRouter(t)
	defer cleanup()
	httpUpstream.err = fmt.Errorf("read spool request body: %w", service.ErrRequestBodySpool)
...
func TestOpenAIGatewayHandler_ResponsesClientReadFailureReturns400(t *testing.T) {
	gin.SetMode(gin.TestMode)
	router, _, httpUpstream, cleanup := newOpenAIResponsesRequestBodyTestRouter(t)
	defer cleanup()
	...
	req.Body = io.NopCloser(requestBodyErrorReader{err: errors.New("client read failed")})
	router.ServeHTTP(rec, req)
...
type requestBodyErrorReader struct{ err error }

func (r requestBodyErrorReader) Read([]byte) (int, error) { return 0, r.err }
...
func TestOpenAIGatewayHandler_ChatCompletionsUsageTaskUsesCapturedEndpointAndSnapshot(t *testing.T) {
	...
	case log := <-usageRepo.created:
		...
		requireRequestPreviewSnapshot(t, log.DetailSnapshot.RequestBody, firstBody)
...
func TestOpenAIGatewayHandler_RetryPathStoresLastOutboundRequestOnly(t *testing.T) {
	...
	upstreamPreview := gjson.Get(usageRepo.lastLog.DetailSnapshot.UpstreamRequestBody, "preview").String()
	require.NotContains(t, upstreamPreview, `"encrypted_content":"secret"`)
	require.Contains(t, upstreamPreview, `"type":"input_text"`)
}
...
func requireRequestPreviewSnapshot(t *testing.T, snapshot string, wantPreview string) {
	t.Helper()
	require.True(t, gjson.Valid(snapshot), "request snapshot must be JSON")
...
func newOpenAIResponsesRequestBodyTestRouter(t *testing.T) (*gin.Engine, *openAIChatCompletionsUsageLogRepoStub, *openAIRetryTrackingHTTPUpstreamStub, func()) {
	...
	httpUpstream := &openAIRetryTrackingHTTPUpstreamStub{responses: []*http.Response{{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"id":"resp_123","object":"response","status":"completed","model":"gpt-5.4","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}`)),
	}}}
	accountRepo := &openAIChatCompletionsAccountRepoStub{account: account}
	concurrencyService := service.NewConcurrencyService(openAIChatCompletionsConcurrencyCacheStub{})
	billingCacheService := service.NewBillingCacheService(nil, nil, nil, nil, nil, nil, cfg, nil)
	...
	return router, usageRepo, httpUpstream, func() { billingCacheService.Stop() }
}
...
func (s *openAIRetryAccountRepoStub) ListModelAvailabilityCandidates(_ context.Context, _ *int64, platforms []string, _ bool) ([]service.Account, error) {
...
func (s *openAIRetryTrackingHTTPUpstreamStub) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	return s.Do(req, proxyURL, accountID, accountConcurrency)
}
...
func (openAIChatCompletionsConcurrencyCacheStub) CleanupExpiredAccountSlotKeys(context.Context) error {
	return nil
}
...
func (openAIChatCompletionsGatewayCacheStub) SetReasoningContent(context.Context, string, string, time.Duration) error {
	return nil
}

func (openAIChatCompletionsGatewayCacheStub) GetReasoningContent(context.Context, string) (string, error) {
