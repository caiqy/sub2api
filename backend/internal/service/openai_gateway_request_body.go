...
const openAIRequestBodyHandleContextKey = "openai_request_body_handle"
...
func getOpenAIRequestBodyHandle(c *gin.Context) *RequestBodyHandle {
	...
	value, _ := c.Get(openAIRequestBodyHandleContextKey)
	handle, _ := value.(*RequestBodyHandle)
	return handle
}
...
func openAIRequestBodyHandleMatchesBytes(handle *RequestBodyHandle, body []byte) bool {
	...
	return handle.Hash() == fmt.Sprintf("%x", sum)
}
...
func openAIUpstreamRequestBodyPreview(req *http.Request, fallback []byte) string {
	if req != nil {
		if handle, ok := req.Context().Value(openAIOwnedBodyHandleContextKey{}).(*RequestBodyHandle); ok {
			return handle.PreviewString()
		}
	}
	return RequestBodyPreviewString(fallback)
}
...
func parseOpenAIWithSourceBodyArgs(bodyHandleOrToken any, args []any) (*RequestBodyHandle, string, bool, string, bool, error) {
	if token, ok := bodyHandleOrToken.(string); ok {
		if len(args) == 3 {
			stream, ok1 := args[0].(bool)
			promptCacheKey, ok2 := args[1].(string)
			codex, ok3 := args[2].(bool)
			if ok1 && ok2 && ok3 {
				return nil, token, stream, promptCacheKey, codex, nil
			}
		}
	} else if handle, ok := bodyHandleOrToken.(*RequestBodyHandle); ok && len(args) == 4 {
		token, ok1 := args[0].(string)
		stream, ok2 := args[1].(bool)
		promptCacheKey, ok3 := args[2].(string)
		codex, ok4 := args[3].(bool)
		if ok1 && ok2 && ok3 && ok4 {
			return handle, token, stream, promptCacheKey, codex, nil
		}
	}
	return nil, "", false, "", false, fmt.Errorf("invalid OpenAI request builder arguments")
}

func (s *OpenAIGatewayService) buildUpstreamRequestWithSourceBody(ctx context.Context, c *gin.Context, account *Account, sourceBody, body []byte, bodyHandleOrToken any, args ...any) (*http.Request, error) {
	bodyHandle, token, _, _, _, err := parseOpenAIWithSourceBodyArgs(bodyHandleOrToken, args)
	if err != nil {
		return nil, err
	}
	if bodyHandle != nil {
