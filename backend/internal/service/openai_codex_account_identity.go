...
func isCodexFingerprintExplicitlyOff(account *Account) bool {
	if account == nil || !account.IsOpenAIOAuthLike() || account.Extra == nil {
		return false
	}
	raw, ok := account.Extra[codexFingerprintModeExtraKey].(string)
	return ok && strings.EqualFold(strings.TrimSpace(raw), string(codexFingerprintOff))
}
...
func applyCodexAccountIdentityClientMetadataMap(requestBody map[string]any, account *Account, apiKeyID int64) bool {
	if requestBody == nil || isCodexFingerprintExplicitlyOff(account) || codexAccountIdentityNamespace(account) == "" {
		return false
	}
	changed := false
...
func applyCodexAccountIdentityClientMetadataRaw(body []byte, account *Account, apiKeyID int64) ([]byte, bool, error) {
	if len(body) == 0 || isCodexFingerprintExplicitlyOff(account) || codexAccountIdentityNamespace(account) == "" {
...
func applyCodexAccountIdentityHeaders(headers http.Header, account *Account, apiKeyID int64) {
	if headers == nil || isCodexFingerprintExplicitlyOff(account) || codexAccountIdentityNamespace(account) == "" {
		return
	}
	for _, field := range codexAccountIdentityFields {
