...
// rememberOpenCodeInboundBody keeps only the session hint needed after protocol conversion.
func rememberOpenCodeInboundBody(c *gin.Context, body []byte) {
	...
	if sessionID := sanitizeSessionID(openCodeSessionIDFromPayload(body)); sessionID != "" {
		c.Set(openCodeInboundBodyContextKey, sessionID)
	}
}
...
func openCodeInboundBodies(c *gin.Context) [][]byte {
	...
	return [][]byte{openCodeSessionHintBody(sessionID)}
}
