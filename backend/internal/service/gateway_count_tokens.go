...
func (s *GatewayService) forwardCountTokensAnthropicAPIKeyPassthrough(ctx context.Context, c *gin.Context, account *Account, body []byte) error {
	...
	upstreamReq, wireBody, err := s.buildCountTokensRequestAnthropicAPIKeyPassthrough(ctx, c, account, body, body, token)
	if err != nil {
		s.countTokensError(c, http.StatusInternalServerError, "api_error", "Failed to build request")
		return err
	}
...
func (s *GatewayService) buildCountTokensRequestAnthropicAPIKeyPassthrough(
	...
	body := sourceBody
	var token string
	switch value := bodyOrToken.(type) {
	case string:
		if len(tokenArgs) != 0 {
			return nil, nil, fmt.Errorf("invalid anthropic count tokens request builder arguments")
		}
		token = value
	case []byte:
		if len(tokenArgs) != 1 {
	...
	body, err := applyAccountPassthroughFieldsWithContext(ctx, account, inboundHeader, sourceBody, body, overrides)
	if err != nil {
		return nil, nil, err
	}
