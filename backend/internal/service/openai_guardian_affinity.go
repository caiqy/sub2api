...
func (s *OpenAIGatewayService) resolveOpenAIGuardianParentAccountID(ctx context.Context, groupID *int64) int64 {
	...
	primaryKey := s.openAISessionCacheKey(affinity.currentSessionHash)
	if primaryKey == "" {
		return 0
	}
	accountID, err := s.cache.GetSessionAccountID(lookupCtx, derefGroupID(groupID), primaryKey)
	...
	legacyKey := s.openAILegacySessionCacheKey(lookupCtx, affinity.currentSessionHash)
	if legacyKey == "" {
		return 0
	}
	accountID, err = s.cache.GetSessionAccountID(lookupCtx, derefGroupID(groupID), legacyKey)
	if err != nil || accountID <= 0 {
		return 0
	}
	return accountID
