...
func Relay(
	...
	resolveTurnStartedAt := func() time.Time {
		...
		if options.TakeNextTurnStartedAt != nil {
			if startedAt := options.TakeNextTurnStartedAt(); !startedAt.IsZero() {
				return startedAt
			}
		}
		return nowFn()
	}
	turnPermit := make(chan struct{}, 1)
	turnPermit <- struct{}{}
	isResponseCreate := func(payload []byte) bool {
		return strings.TrimSpace(gjson.GetBytes(payload, "type").String()) == "response.create"
	}
	acquireTurn := func(payload []byte) (bool, error) {
		if !isResponseCreate(payload) {
			return false, nil
		}
		select {
		case <-relayCtx.Done():
			return false, relayCtx.Err()
		case <-turnPermit:
			return true, nil
		}
	}
	releaseTurn := func() {
		select {
		case turnPermit <- struct{}{}:
		default:
		}
	}
	writeTurnUpstream := func(msgType coderws.MessageType, payload []byte) error {
		held, err := acquireTurn(payload)
		if err != nil {
			return err
		}
		if held {
	...
	if options.FirstMessageSent {
		held, err := acquireTurn(firstClientMessage)
		if err != nil {
		...
		if err := writeTurnUpstream(firstMessageType, firstClientMessage); err != nil {
		...
		if isResponseCreate(firstClientMessage) {
			turn := <-turnStartCh
			startTurn(turn.payload, turn.startAt)
		}
		emitRelayTrace(onTrace, RelayTraceEvent{
...
func runUpstreamToClient(
	...
	releaseTurn func(),
	upstreamReadDoneCallbacks ...chan<- struct{},
) {
	...
	if len(upstreamReadDoneCallbacks) > 0 {
		upstreamReadDone = upstreamReadDoneCallbacks[0]
	}
	go func() {
		if upstreamReadDone != nil {
			defer close(upstreamReadDone)
		}
...
func observeUpstreamMessage(
	...
	if state.firstTokenMs == nil && recordsFirstToken && activeMatches {
		...
		if state.activeTurn.firstTokenMs == nil {
...
func finalizeObservedRelayTerminal(state *relayState, observed observedUpstreamEvent, now time.Time) observedUpstreamEvent {
	...
	if responseID != "" {
		...
		observed.completedActiveTurn = timing != nil && (state.activeTurn == nil || state.activeTurn == timing)
		...
		observed.completedActiveTurn = true
...
func emitTurnComplete(
	...
	if !observed.completedActiveTurn && strings.TrimSpace(observed.responseID) != "" {
		return
	}
	responseID := strings.TrimSpace(observed.responseID)
...
func openAIWSRelayGetOrInitTurnTiming(state *relayState, responseID string, now time.Time) *relayTurnTiming {
	...
	if !ok || timing == nil || timing.startAt.IsZero() {
		...
		if active != nil && active.responseID != "" && active.responseID != responseID {
			return nil
		}
		if active != nil && !active.startAt.IsZero() {
			timing = active
