...
func applyOpenAIReasoningEffortPolicy(body []byte, maxEffort string, mappings []ReasoningEffortMapping, overLimit string, requestModel string) ([]byte, bool, error) {
...
func applyOpenAIWSReasoningEffortPolicy(payload []byte, hooks *OpenAIWSIngressHooks, requestModel string) ([]byte, error) {
	...
	capped, changed, err := applyOpenAIReasoningEffortPolicy(payload, hooks.MaxReasoningEffort, hooks.ReasoningEffortMappings, hooks.MaxReasoningEffortOverLimit, requestModel)
	if err != nil {
		return payload, err
	}
	if changed {
