...
func (m *mockAccountRepoForPlatform) ListTempUnschedulableByPlatform(ctx context.Context, platform string, now time.Time) ([]Account, error) {
	var result []Account
	for _, acc := range m.accounts {
...
func TestGatewayService_SelectAccountWithLoadAwareness(t *testing.T) {
	...
	t.Run("Gemini sticky 命中不受 Anthropic 开关影响", func(t *testing.T) {
		...
		result, err := svc.SelectAccountWithLoadAwareness(ctx, &groupID, "gemini-sticky-hit", "gemini-2.5-pro", nil, "", int64(0))
		require.NoError(t, err)
		require.NotNil(t, result)
