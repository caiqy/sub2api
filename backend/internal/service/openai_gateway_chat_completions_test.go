...
func TestForwardAsChatCompletions_APIKeyAutoInjectsPromptCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
...
func TestForwardAsChatCompletions_ResponsesShapeUsesInputForAutoCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-4o","input":[{"role":"user","content":[{"type":"input_text","text":"hello","nonce":9007199254740993}]}],"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Set("api_key", &APIKey{ID: 99})
	...
	require.Error(t, err)
	require.Nil(t, result)
	want := isolateOpenAISessionID(99, deriveAutoOpenAICompatPromptCacheKey(body, "gpt-4o"))
...
func TestForwardAsChatCompletions_ResponsesShapeDoesNotUseModelOnlyCacheKey(t *testing.T) {
	gin.SetMode(gin.TestMode)

	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	body := []byte(`{"model":"gpt-4o","input":[],"stream":false}`)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/chat/completions", bytes.NewReader(body))
	c.Request.Header.Set("Content-Type", "application/json")
...
func TestForwardAsChatCompletions_ResponsesShapeAutoDerivesDistinctIsolatedPromptCacheKeys(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	firstKey := isolateOpenAISessionID(99, deriveAutoOpenAICompatPromptCacheKey(firstBody, "gpt-5.4"))
	secondKey := isolateOpenAISessionID(99, deriveAutoOpenAICompatPromptCacheKey(secondBody, "gpt-5.4"))
	require.NotEqual(t, firstKey, secondKey)
	require.Equal(t, firstKey, gjson.GetBytes(upstream.bodies[0], "prompt_cache_key").String())
	require.Equal(t, generateSessionUUID(firstKey), upstream.requests[0].Header.Get("session_id"))
	require.Equal(t, secondKey, gjson.GetBytes(upstream.bodies[1], "prompt_cache_key").String())
	require.Equal(t, generateSessionUUID(secondKey), upstream.requests[1].Header.Get("session_id"))
