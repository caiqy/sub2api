...
func (s *OpenAIGatewayService) ForwardGrokVoice(ctx context.Context, c *gin.Context, account *Account, endpoint string, body []byte, contentType string) (*OpenAIForwardResult, error) {
	...
	SetUsageResponseSize(c, int64(len(data)))
...
func (s *OpenAIGatewayService) ProxyGrokRealtime(ctx context.Context, c *gin.Context, client *coderws.Conn, account *Account, token, model string, eventGuard func(context.Context, []byte) error) (string, bool, error) {
	...
	audioObserved, err := s.ProxyGrokRealtimeConn(ctx, c, client, upstream, eventGuard)
	return model, audioObserved, err
}
...
func (s *OpenAIGatewayService) ProxyGrokRealtimeConn(ctx context.Context, c *gin.Context, client *coderws.Conn, upstream *GrokRealtimeUpstream, eventGuards ...func(context.Context, []byte) error) (bool, error) {
	if s == nil || client == nil || upstream == nil || upstream.conn == nil {
		return false, fmt.Errorf("realtime connection is required")
	}
	conn := upstream.conn
	var eventGuard func(context.Context, []byte) error
	if len(eventGuards) > 0 {
		eventGuard = eventGuards[0]
	}
	...
	go func() {
		defer pumps.Done()
		for {
			msg, readErr := conn.ReadMessage(relayCtx)
			if readErr != nil {
	...
	go func() {
		...
		for {
			...
			if writeErr := conn.WriteJSON(relayCtx, raw); writeErr != nil {
				exits <- relayExit{err: writeErr}
				return
			}
		}
	...
	pumps.Wait()
	if first.guard {
		return audioObserved.Load(), first.err
	}
	if second.guard {
		return audioObserved.Load(), second.err
	}
	return audioObserved.Load(), first.err
}
