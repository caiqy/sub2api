...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	...
	if account.Platform == PlatformAnthropic && account.Type == AccountTypeServiceAccount {
		...
		vertexBody, err := io.ReadAll(req.Body)
		if err != nil {
			return nil, nil, err
		}
		req.Body = io.NopCloser(bytes.NewReader(vertexBody))
	...
	if account.Type == AccountTypeAPIKey {
		...
		if baseURL != "" {
			...
			targetURL = buildAnthropicMessagesURL(validatedURL) + "?beta=true"
		}
...
func (s *GatewayService) buildUpstreamRequestWithSourceBody(ctx context.Context, c *gin.Context, account *Account, sourceBody, body []byte, token, tokenType, modelID string, reqStream bool, mimicClaudeCode bool) (*http.Request, []byte, error) {
	inbound := http.Header{}
	if c != nil && c.Request != nil {
		inbound = c.Request.Header
	}
	overrides := http.Header{}
	updatedBody, err := applyAccountPassthroughFieldsWithContext(ctx, account, inbound, sourceBody, body, overrides)
	if err != nil {
		return nil, nil, err
	}
	req, wireBody, err := s.buildUpstreamRequest(ctx, c, account, updatedBody, token, tokenType, modelID, reqStream, mimicClaudeCode)
	if err != nil {
		return nil, nil, err
	}
