...
func TestAdminProxyUpdateCredentials(t *testing.T) {
	...
	username, password := "new-user", "new-pass"
	check("omitted", UpdateProxyInput{Status: "inactive"}, "old-user", "old-pass")
	check("clear both", UpdateProxyInput{UsernameSet: true, PasswordSet: true}, "", "")
	check("clear username only", UpdateProxyInput{UsernameSet: true}, "", "old-pass")
	check("clear password only", UpdateProxyInput{PasswordSet: true}, "old-user", "")
