...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurn(
	...
	return s.proxyOpenAIWSHTTPBridgeTurnWithOutboundHook(ctx, c, account, token, payload, payloadBytes, originalModel, imageBillingModel, imageSizeTier, imageInputSize, grokCacheIdentity, turn, writeClientMessage, CanonicalRequestedReasoningEffort(payload), nil)
}
...
func (s *OpenAIGatewayService) proxyOpenAIWSHTTPBridgeTurnWithOutboundHook(
	...
	buildUpstreamRequest := func(requestBody []byte) (*http.Request, error) {
		...
		if account.Platform == PlatformGrok {
			...
			upstreamReq, buildErr = s.buildUpstreamRequestOpenAIPassthrough(upstreamCtx, c, account, requestBody, requestBody, token)
		}
		if buildErr != nil {
			return nil, buildErr
		}
	...
	flushPendingClientMessages := func() error {
		if clientDisconnected || len(pendingClientMessages) == 0 {
			return nil
		}
	...
	for scanner.Scan() {
		...
		if eventType == "error" || eventType == "response.failed" {
			...
			} else if eventType == "error" && imageCounter.Count() > 0 {
				upstreamEventErr = errors.New(errMessage)
			} else if eventType == "error" && !wroteDownstream && !sawResponseCreated && !requestScopedCapacity {
				return resultWithUsage(), NewOpenAIWSClientCloseError(coderws.StatusTryAgainLater, "upstream websocket error", errors.New(errMessage))
		...
		if isOpenAIWSTerminalEvent(eventType) && !bareErrorPending {
			...
			if cyberPolicyForwarded {
				return resultWithUsage(), errOpenAICyberPolicyForwarded
			}
