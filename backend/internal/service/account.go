...
func (a *Account) GetCodingPlanProvider() string {
	...
	if err != nil || !strings.EqualFold(baseURL.Scheme, "https") || baseURL.User != nil {
		return ""
	}
	host := strings.ToLower(baseURL.Hostname())
	officialHost := func(domain string) bool {
		return host == domain || strings.HasSuffix(host, "."+domain)
	}
	switch {
	case host == "api.kimi.com" && (baseURL.Path == "/coding" || strings.HasPrefix(baseURL.Path, "/coding/")):
		return PlatformKimi
	case officialHost("bigmodel.cn"), host == "api.z.ai":
		return PlatformZhipu
	case officialHost("minimax.io"), officialHost("minimaxi.com"), officialHost("minimax.com"):
