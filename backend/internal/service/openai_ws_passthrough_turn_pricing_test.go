...
func TestPassthroughIngressFollowUpCallsBeforeTurnAfterBeforeRequest(t *testing.T) {
	...
	upstream.Send(`{"type":"response.created","response":{"id":"resp_pricing_1","model":"gpt-5.1"}}`)
...
func testPassthroughIngressFreezesSubsequentTurnBeforeRequestPolicy(t *testing.T, secondMessageType coderws.MessageType) {
	...
	firstCreated, err := readPassthroughLifecycleFrame(t, clientConn, 3*time.Second)
	require.NoError(t, err)
	require.Equal(t, "response.created", gjson.GetBytes(firstCreated, "type").String())
