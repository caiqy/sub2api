...
func (s *OpenAIGatewayService) ForwardResponsesInputTokens(
	...
	upstreamBodyHandle, err := NewRequestBodyHandleFromBytes(upstreamBody, RequestBodyHandleOptions{})
	if err != nil {
		writeOpenAIResponsesInputTokensError(c, http.StatusServiceUnavailable, "api_error", "Failed to prepare request body")
		return fmt.Errorf("responses input_tokens: prepare upstream request body: %w", err)
	}
	defer CleanupRequestBodyHandle(upstreamBodyHandle)
	upstreamReq, err := s.buildInputTokensUpstreamRequest(ctx, c, account, upstreamBodyHandle, token)
	if err != nil {
