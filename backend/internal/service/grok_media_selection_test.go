...
func TestGrokVideoStickySelectionIgnoresHealthEscape(t *testing.T) {
	...
	selection, updatedReq, err := scheduler.selectBySessionHash(context.Background(), req)
	require.NoError(t, err)
	require.Nil(t, selection)
	require.True(t, updatedReq.SkipStickyBind)
	req.DisableStickyEscape = true
	selection, updatedReq, err = scheduler.selectBySessionHash(context.Background(), req)
	require.NoError(t, err)
	require.False(t, updatedReq.SkipStickyBind)
	require.True(t, selection.Acquired)
	selection.ReleaseFunc()
}
