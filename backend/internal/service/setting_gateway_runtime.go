...
func (s *SettingService) GetGatewayRuntimeSettings(ctx context.Context) (*GatewayRuntimeSettings, error) {
	...
	settings.OpenAITextFirstTokenTimeout = s.cfg.Gateway.OpenAITextFirstTokenTimeout
	settings.OpenAIImageFirstTokenTimeout = s.cfg.Gateway.OpenAIImageFirstTokenTimeout
...
func (s *SettingService) loadGatewayRuntimeSettingsFromDB(ctx context.Context) {
	...
	if json.Unmarshal([]byte(value), &settings) != nil || json.Unmarshal([]byte(value), &raw) != nil {
		return
	}
...
func (s *SettingService) syncUsageLogDetailRetentionLimitsFromConfig() {
	if s != nil && s.cfg != nil {
		SetUsageLogDetailRetentionLimits(gatewayRuntimeRetentionLimitOrDefault(s.cfg.Gateway.UsageLogDetailRetentionLimit), gatewayRuntimeRetentionLimitOrDefault(s.cfg.Gateway.ImageUsageLogDetailRetentionLimit))
	}
}
...
func applyGatewayOpenAIWSSchedulerMode(target *string, raw string) {
	...
	if target != nil && (mode == "weighted" || mode == "layered") {
		*target = mode
	}
}

func applyPositiveIntSetting(target *int, raw string) {
	if parsed, err := strconv.Atoi(strings.TrimSpace(raw)); target != nil && err == nil && parsed > 0 {
		*target = parsed
	}
}
...
func applyPositiveFloatSettingInRange(target *float64, raw string, minExclusive, maxInclusive float64) {
	if parsed, err := strconv.ParseFloat(strings.TrimSpace(raw), 64); target != nil && err == nil && parsed > minExclusive && parsed <= maxInclusive {
		*target = parsed
	}
}

func applyFloatSettingGreaterThan(target *float64, raw string, minExclusive float64) {
	if parsed, err := strconv.ParseFloat(strings.TrimSpace(raw), 64); target != nil && err == nil && parsed > minExclusive {
		*target = parsed
	}
}
