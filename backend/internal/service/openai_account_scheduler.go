...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if s.service != nil && s.service.openAIStickyEnabled() && previousResponseID != "" && NormalizeOpenAICompatiblePlatform(req.Platform) == PlatformOpenAI &&
		...
		if selection != nil && selection.Account != nil {
			compatible, reason := s.isAccountRequestCompatibleReason(ctx, selection.Account, req)
			...
			if !groupCompatible ||
				!compatible || !accountSatisfiesPrivacyRequirement(selection.Account, schedGroup) ||
				!s.isAccountTransportCompatible(selection.Account, req.RequiredTransport) {
				if !accountSatisfiesPrivacyRequirement(selection.Account, schedGroup) && s.isAccountTransportCompatible(selection.Account, req.RequiredTransport) {
					recordPrivacyRequirementError(ctx, s.service, selection.Account, schedGroup)
				}
				if !groupCompatible || reason == "channel_upstream_restricted" || !s.isAccountTransportCompatible(selection.Account, req.RequiredTransport) {
	...
	if s.service != nil && s.service.openAIStickyEnabled() && !req.StickyWeighted {
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if account == nil {
		if accountBeforeRecheck != nil && s.service != nil && s.service.accountRepo != nil {
			...
			if s.isAccountTransportCompatible(latest, req.RequiredTransport) {
	...
	if s.service.concurrencyService != nil {
		if !req.DisableStickyEscape && s.shouldEscapeBusyStickyAccount(ctx, accountID, cfg) {
			if req.ExcludedIDs == nil {
...
func (s *defaultOpenAIAccountScheduler) recheckSessionStickyAccountFromDB(
	...
	if !s.service.openAIStickyAccountMatchesSchedulingGroup(ctx, latest, req.GroupID) {
		return nil, true
	}
...
func (s *OpenAIGatewayService) openAIStickyAccountMatchesSchedulingGroup(ctx context.Context, account *Account, groupID *int64) bool {
	if account == nil {
		return false
	}
	...
	if groupID == nil || len(account.GroupIDs) > 0 || len(account.AccountGroups) > 0 || s.accountRepo == nil {
		return false
	}
	accounts, err := s.accountRepo.ListSchedulableByGroupIDAndPlatform(ctx, *groupID, NormalizeOpenAICompatiblePlatform(account.Platform))
	if err != nil {
		return false
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	plan := openAIAccountLoadPlan{
		...
		includeOverflowFallback:   req.UseUpstreamTokenCost,
	}
	if len(candidates) == 0 {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for _, candidate := range selectionOrder {
		if candidate.account == nil || (candidate.loadKnown && candidate.account.Concurrency > 0 && candidate.loadInfo.CurrentConcurrency >= candidate.account.Concurrency) {
			continue
		}
		...
		if fresh.Concurrency != candidate.account.Concurrency {
			...
			if !attempted || result == nil || !result.Acquired {
				continue
			}
		}
		if req.SessionHash != "" && !req.PreserveStickyBinding {
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAIAccountSlot(ctx context.Context, accountID int64, maxConcurrency int, budget *openAISelectionProbeBudget) (*AcquireResult, bool, error) {
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		if account == nil || !accountSatisfiesPrivacyRequirement(account, schedGroup) || !s.isAccountRequestCompatible(ctx, account, req) || !s.isAccountTransportCompatible(account, req.RequiredTransport) {
			if !accountSatisfiesPrivacyRequirement(account, schedGroup) && s.isAccountTransportCompatible(account, req.RequiredTransport) {
		...
		if !s.service.openAIStickyAccountMatchesSchedulingGroup(ctx, account, req.GroupID) {
...
func isOpenAIAccountRequestCompatible(ctx context.Context, service *OpenAIGatewayService, account *Account, req OpenAIAccountScheduleRequest) bool {
	compatible, _ := openAIAccountRequestCompatibilityReason(ctx, service, account, req)
	return compatible
}
...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	return openAIAccountRequestCompatibilityReason(ctx, service, account, req)
}

func openAIAccountRequestCompatibilityReason(ctx context.Context, service *OpenAIGatewayService, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	if account == nil {
...
func (s *OpenAIGatewayService) openAIAdvancedSchedulerRuntimeSettings(ctx context.Context) openAIAdvancedSchedulerRuntimeSettings {
	...
	fromCache := func(cached *cachedOpenAIAdvancedSchedulerSetting) openAIAdvancedSchedulerRuntimeSettings {
		return openAIAdvancedSchedulerRuntimeSettings{
			...
			enabled:                        modeRequested || cached.enabled,
	...
	result, _, _ := openAIAdvancedSchedulerSettingSF.Do(openAIAdvancedSchedulerSettingKey, func() (any, error) {
		...
		if repo := s.openAIAdvancedSchedulerSettingRepo(); repo != nil {
			...
			if values, err := repo.GetMultiple(dbCtx, openAIAdvancedSchedulerRuntimeSettingKeys()); err == nil {
				...
				dbEnabled = strings.EqualFold(strings.TrimSpace(values[openAIAdvancedSchedulerSettingKey]), "true")
				...
				dbEnabled = strings.EqualFold(strings.TrimSpace(fallbackValues[openAIAdvancedSchedulerSettingKey]), "true")
		...
		enabled := modeRequested || dbEnabled
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	// ponytail: merge bridge for the two pre-existing call forms; remove after all callers use the typed tag signature.
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, "", excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	schedulerSessionHash := sessionHash
	schedulerStickyAccountID := stickyAccountID
	if preserveGuardianParentBinding {
		// The layered fork predates PreserveStickyBinding and would delete a
		// guardian parent binding on a child/hash collision. Skip that lookup.
		schedulerSessionHash = ""
		schedulerStickyAccountID = 0
	}
	...
	if scheduler != nil && guardianParentAccountID > 0 {
		guardianScheduler := &defaultOpenAIAccountScheduler{service: s, stats: s.openaiAccountStats}
		selection, _, err := guardianScheduler.selectBySessionHash(ctx, OpenAIAccountScheduleRequest{
			GroupID:                 groupID,
			Platform:                platform,
			SessionHash:             sessionHash,
			StickyAccountID:         guardianParentAccountID,
	...
	for scheduler != nil {
		selection, nextDecision, err := scheduler.Select(ctx, OpenAIAccountScheduleRequest{
			GroupID:                 groupID,
			Platform:                platform,
			SessionHash:             schedulerSessionHash,
			StickyAccountID:         schedulerStickyAccountID,
...
func accountSupportsOpenAICapabilities(account *Account, requiredCapability OpenAIEndpointCapability, requiredImageCapability OpenAIImagesCapability) bool {
	...
	return account.SupportsOpenAIEndpointCapability(requiredCapability) && account.SupportsOpenAIImageCapability(requiredImageCapability)
}
...
func buildOpenAIAccountSchedulerScoreSnapshot(
	...
	if weights.UpstreamCost > 0 {
		accountList := make([]*Account, 0, len(candidates))
		for _, candidate := range candidates {
			accountList = append(accountList, candidate.account)
		}
		upstreamCostFactors = openAIUpstreamCostFactors(accountList, now, oauthSchedulingRateMultiplier)
	}
	if weights.Reset > 0 {
		for _, candidate := range candidates {
			end, ok := openAISchedulingResetWindowEnd(candidate.account, now)
