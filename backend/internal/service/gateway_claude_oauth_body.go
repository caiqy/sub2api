...
func extractSystemTextAndCacheControl(system any, preserveBillingInstruction bool) (string, any, bool) {
	switch v := system.(type) {
	case string:
		text := strings.TrimSpace(v)
		return text, nil, strings.Contains(text, claudeCodeBillingHeaderPrefix)
	case []any:
		var parts []string
		...
		return strings.Join(parts, "\n\n"), cacheControl, hadBillingAttribution
	default:
		return "", nil, false
	}
}
...
func rewriteSystemForNonClaudeCodeWithPromptBlocks(body []byte, system any, expansionPrompt string, blocksConfig string, preserveBillingInstruction bool) []byte {
	...
	originalSystemText, originalSystemCacheControl, hadBillingAttribution := extractSystemTextAndCacheControl(system, preserveBillingInstruction)
	...
	if hadBillingAttribution && strings.TrimSpace(blocksConfig) == "" && len(systemBlocks) == 3 {
		...
		systemBlocks = [][]byte{systemBlocks[0], identity}
	}
