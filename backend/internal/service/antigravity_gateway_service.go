...
func (p *antigravityRetryLoopParams) payload() (*RequestBodyHandle, error) {
	...
	p.body, p.payloadHandle, p.ownedPayload = nil, handle, true
	return handle, nil
}
...
func newAntigravityPayloadRequest(p *antigravityRetryLoopParams, baseURL string) (*http.Request, error) {
	...
	req.Body, req.ContentLength, req.GetBody = body, handle.Size(), handle.Open
	return req, nil
}
...
func (s *AntigravityGatewayService) getAntigravityAccessToken(ctx context.Context, account *Account) (string, error) {
...
func applyAntigravityPassthrough(ctx context.Context, c *gin.Context, account *Account, sourceBody, targetBody []byte) ([]byte, http.Header, error) {
	inbound := http.Header{}
	if c != nil && c.Request != nil {
		inbound = c.Request.Header
	}
	outbound := http.Header{}
	body, err := applyAccountPassthroughFieldsWithContext(ctx, account, inbound, sourceBody, targetBody, outbound)
	return body, outbound, err
}
