...
func TestOpenAIResponsesPassthroughClampsAfterFieldRules(t *testing.T) {
	gin.SetMode(gin.TestMode)

	account := ollamaUpstreamTestAccount(PlatformDeepseek, 339)
	account.Extra["passthrough_fields_enabled"] = true
	account.Extra["passthrough_field_rules"] = []PassthroughFieldRule{
		{Target: "body", Mode: "inject", Key: "model", Value: "deepseek-v4-flash"},
		{Target: "body", Mode: "forward", Key: "max_output_tokens"},
	}
	sourceBody := []byte(`{"model":"client-alias","max_output_tokens":256000,"input":"hi"}`)
	targetBody := []byte(`{"model":"other-model","max_output_tokens":64,"input":"hi"}`)
	c := adaptiveProtocolTestContext("/v1/responses", sourceBody)
	svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig()}

	req, err := svc.buildUpstreamRequestOpenAIPassthrough(context.Background(), c, account, sourceBody, targetBody, "sk-test")
	...
	require.Equal(t, int64(65535), gjson.GetBytes(wireBody, "max_output_tokens").Int())
}

func TestForwardResponsesPassthroughDoesNotClampReplacedNonDeepSeekModel(t *testing.T) {
	gin.SetMode(gin.TestMode)

	account := ollamaUpstreamTestAccount(PlatformDeepseek, 340)
	account.Credentials["api_protocol"] = APIProtocolResponses
	account.Extra["passthrough_fields_enabled"] = true
	account.Extra["passthrough_field_rules"] = []PassthroughFieldRule{
		{Target: "body", Mode: "inject", Key: "model", Value: "qwen3-coder"},
	}
	body := []byte(`{"model":"deepseek-v4-flash","max_output_tokens":256000,"input":"hi","stream":false}`)
	upstream := &httpUpstreamRecorder{err: errors.New("stop after capture")}
	svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig(), httpUpstream: upstream}

	_, err := svc.Forward(context.Background(), adaptiveProtocolTestContext("/v1/responses", body), account, body)
	require.Error(t, err)
	require.Equal(t, "qwen3-coder", gjson.GetBytes(upstream.lastBody, "model").String())
	require.Equal(t, int64(256000), gjson.GetBytes(upstream.lastBody, "max_output_tokens").Int())
}
