...
func TestProxyOpenAIWSHTTPBridgeTurn_RequestedReasoningEffortUsesCurrentTurnBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{
			name:      "terminal",
			secondSSE: `data: {"type":"response.failed","response":{"id":"resp_second","status":"failed","error":{"code":"server_error","message":"terminal failure"},"usage":{"input_tokens":2,"output_tokens":1}}}` + "\n\n",
		},
		{
			name: "exhausted",
			secondSSE: strings.Join([]string{
				...
				"",
				`data: {"type":"response.output_text.delta","response":{"id":"resp_second"},"delta":"partial"}`,
				"",
				`data: {"type":"response.failed","response":{"id":"resp_second","status":"failed","error":{"code":"usage_limit_reached","type":"rate_limit_error","message":"The usage limit has been reached"},"usage":{"input_tokens":2,"output_tokens":1}}}`,
				"",
			}, "\n"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			firstSSE := `data: {"type":"response.completed","response":{"id":"resp_first","model":"gpt-5.4","usage":{"input_tokens":1,"output_tokens":1}}}` + "\n\n"
			upstream := &httpUpstreamRecorder{responses: []*http.Response{
				...
				{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}}, Body: io.NopCloser(strings.NewReader(tt.secondSSE))},
			}}
			svc := &OpenAIGatewayService{
				cfg:          &config.Config{Gateway: config.GatewayConfig{MaxLineSize: defaultMaxLineSize}},
				httpUpstream: upstream,
			}
			account := &Account{ID: 5883, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1}
			recorder := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(recorder)
			c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
			firstPayload := []byte(`{"type":"response.create","model":"gpt-5.4-max","reasoning":{"effort":"high"}}`)
			firstResult, err := svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, account, "test-token", firstPayload, len(firstPayload),
				"gpt-5.4-max", "", "", "", "", 1, func([]byte) error { return nil },
			)
			require.NoError(t, err)
			require.NotNil(t, firstResult)
			require.NotNil(t, firstResult.RequestedReasoningEffort)
			require.Equal(t, "high", *firstResult.RequestedReasoningEffort)
			require.NotNil(t, firstResult.ReasoningEffort)
			require.Equal(t, "high", *firstResult.ReasoningEffort)

			secondPayload := []byte(`{"type":"response.create"}`)
			secondResult, err := svc.proxyOpenAIWSHTTPBridgeTurn(
				context.Background(), c, account, "test-token", secondPayload, len(secondPayload),
				"gpt-5.4-max", "", "", "", "", 2, func([]byte) error { return nil },
			)
			require.NoError(t, err)
			require.NotNil(t, secondResult)
			require.Nil(t, secondResult.RequestedReasoningEffort)
			require.NotNil(t, secondResult.ReasoningEffort)
			require.Equal(t, "xhigh", *secondResult.ReasoningEffort)
			require.False(t, gjson.GetBytes(upstream.bodies[1], "model").Exists())
			require.False(t, gjson.GetBytes(upstream.bodies[1], "reasoning").Exists())
		})
	}
}

func TestProxyOpenAIWSHTTPBridgeTurn_ExplicitNilRequestedReasoningEffortWinsOverBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	sse := `data: {"type":"response.completed","response":{"id":"resp_explicit_nil","model":"gpt-5.4-max","usage":{"input_tokens":1,"output_tokens":1}}}` + "\n\n"
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
		Body:       io.NopCloser(strings.NewReader(sse)),
	...
	account := &Account{ID: 5884, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Concurrency: 1}
	payload := []byte(`{"type":"response.create","model":"gpt-5.4-max"}`)
	require.NotNil(t, CanonicalRequestedReasoningEffort(payload), "body model suffix must be distinguishable from explicit nil")
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "gpt-5.4-max", gjson.GetBytes(upstream.lastBody, "model").String())
...
func TestProxyOpenAIWSHTTPBridgeTurnStagesMetadataBeforeLaterTurnRateLimitFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_later_shed"}}`,
		"",
		`data: {"type":"error","error":{"code":"rate_limit_error","message":"rate limited"}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"X-Request-Id": []string{"rid-ws-bridge-later-capacity"}},
		Body:       io.NopCloser(strings.NewReader(body)),
	}}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	payload := []byte(`{"type":"response.create","model":"gpt-5","input":"continue"}`)
	var writes [][]byte
...
func TestProxyOpenAIWSHTTPBridgeTurnMarksCyberPolicyFailure(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := strings.Join([]string{
		`data: {"type":"response.created","response":{"id":"resp_cyber_bridge"}}`,
		"",
		`data: {"type":"response.failed","response":{"id":"resp_cyber_bridge","status":"failed","error":{"code":"cyber_policy","message":"blocked by cyber policy"},"usage":{"input_tokens":23,"output_tokens":5}}}`,
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"X-Request-Id": []string{"rid-ws-bridge-cyber"}},
		Body:       io.NopCloser(strings.NewReader(body)),
	}}
	svc := &OpenAIGatewayService{cfg: &config.Config{}, httpUpstream: upstream}
	...
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/responses", nil)
	payload := []byte(`{"type":"response.create","model":"gpt-5","input":"blocked"}`)
	var writes [][]byte
