...
func resolveOpenAIAttemptFingerprintIDs(c *gin.Context, account *Account, body []byte) *codexFingerprintIDs {
	...
	if c != nil && account != nil {
		...
		if accountMatches && cachedAccountID == account.ID && body == nil {
			if cached, ok := c.Get(codexFingerprintCachedIDsContextKey); ok {
				if cached == nil {
					return nil
				}
				if ids, ok := cached.(*codexFingerprintIDs); ok {
					if ids == nil || ids.mode == account.GetCodexFingerprintMode() {
						return ids
					}
				}
			}
	...
	if c != nil && account != nil {
		...
		c.Set(codexFingerprintCachedIDsContextKey, ids)
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	var lineageEntryDigests []string
	if bytes.Contains(body, []byte(`"encrypted_content"`)) {
		lineageEntryDigests = collectOpenAIEncryptedContentDigestsRaw(body)
	}
	lineageSessionHash := ""
	if stateStore := s.getOpenAIWSStateStore(); stateStore != nil && (stateStore.HasAnySessionInvalidEncryptedContent() || len(lineageEntryDigests) > 0) {
	...
	if wsDecision.Transport == OpenAIUpstreamTransportResponsesWebsocketV2 {
		...
		recoverInvalidEncryptedContent := func(attempt int) bool {
			...
			if len(lineageEntryDigests) > 0 {
				s.markOpenAIWSInvalidEncryptedContentLineage(lineageGroupID, lineageSessionHash, lineageEntryDigests)
			}
			previousResponseID := openAIWSPayloadString(wsReqBody, "previous_response_id")
	...
	passthroughSourceBodyNeeded := false
	if enabled, rules, rulesErr := AccountPassthroughFieldRules(account); rulesErr != nil {
		return nil, rulesErr
	} else if enabled {
		for _, rule := range rules {
			if rule.Target == "body" && (rule.Mode == "map" || rule.Mode == "forward") {
				passthroughSourceBodyNeeded = true
				break
			}
		}
	}
	var sourceHandle *RequestBodyHandle
	var sourceHandleOwned bool
	if passthroughSourceBodyNeeded {
		sourceHandle, sourceHandleOwned, err = openAIRequestBodyHandleForContext(c, originalBody)
		if err != nil {
			return nil, err
		}
		defer func() {
			if sourceHandleOwned {
				CleanupRequestBodyHandle(sourceHandle)
			}
		}()
	}
	...
	for {
		...
		sourceBody := firstSourceBody
		firstSourceBody = nil
		if passthroughSourceBodyNeeded && len(sourceBody) == 0 {
		...
		upstreamReq, err := s.buildUpstreamRequestWithSourceBody(upstreamCtx, c, account, sourceBody, attemptBody, attemptHandle, token, reqStream, promptCacheKey, isCodexCLI)
		if headerGuard == nil {
			releaseUpstreamCtx()
		}
		if err != nil {
		...
		if resp.StatusCode >= 400 {
			...
			makeFailoverError := func() error {
				...
				retryableOnSameAccount := requestScopedTransient || (!shouldDisable && account.IsPoolMode() && (account.IsPoolModeRetryableStatus(resp.StatusCode) || isOpenAITransientProcessingError(resp.StatusCode, upstreamMsg, respBody)))
			...
			if !httpInvalidEncryptedContentRetryTried && resp.StatusCode == http.StatusBadRequest && upstreamCode == "invalid_encrypted_content" {
				...
				requestView = newOpenAIRequestView(requestBody)
				reqBody = nil
				decoded, decodeErr := ensureReqBody()
				if decodeErr != nil {
					return nil, decodeErr
			...
			if resp.StatusCode == http.StatusBadRequest {
				...
				retryBody, reason, changed, retryErr := normalizeOpenAIResponsesRejectedFieldRetryBody(resp.StatusCode, requestBody, respBody)
				if retryErr != nil {
			...
			if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) || isOpenAIUpstreamCapacityShedEvent(respBody) {
		...
		if reqStream {
			...
			if err != nil {
				if signal, ok := asOpenAICompactFallbackSignal(err); ok {
					if retryBody, fallbackModel, retry := s.prepareOpenAICompactFallbackRetry(
						c, account, requestedModel, requestBody, http.StatusBadRequest, signal.message, signal.payload, compactModelFallbackRetried,
					); retry {
					...
					return s.handleErrorResponse(ctx, compactResp, c, account, requestBody, resolveOpenAIErrorSchedulingModel(billingModel, upstreamModel))
				}
				if streamResult != nil {
		...
		forwardResult := buildForwardResult(usage, firstTokenMs, responseID, imageCount, imageOutputSizes)
