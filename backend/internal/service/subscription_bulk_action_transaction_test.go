...
func (r *transactionalBulkSubscriptionRepo) ResetUsageWindowsWithVersion(ctx context.Context, id int64, daily, weekly, monthly bool, dailyStart, periodicStart time.Time) (int64, error) {
	return time.Now().UnixNano(), r.ResetUsageWindows(ctx, id, daily, weekly, monthly, dailyStart, periodicStart)
}
