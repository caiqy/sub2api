...
func openAIWSPassthroughFastPolicyModel(account *Account, model string, payload []byte) string {
	model = strings.TrimSpace(model)
	if account == nil || model == "" || strings.TrimSpace(gjson.GetBytes(payload, "model").String()) != "" {
		return model
	}
	if mapped := strings.TrimSpace(normalizeOpenAIModelForUpstream(account, account.GetMappedModel(model))); mapped != "" {
		return mapped
	}
	return model
}
...
func (m *openAIWSPassthroughUsageMeta) captureRequestedReasoningEffort(originalBody []byte, modelCandidates ...string) {
	...
	m.requestedReasoningEffort.Store(CanonicalRequestedReasoningEffort(originalBody, modelCandidates...))
}
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	updatedFirst, blocked, policyErr := s.applyOpenAIFastPolicyToWSResponseCreate(ctx, account, openAIWSPassthroughFastPolicyModel(account, capturedSessionModel, firstClientMessage), firstClientMessage)
	if policyErr != nil {
	...
	if relayExit == nil {
		switch strings.TrimSpace(relayResult.TerminalEventType) {
		case "error", "response.failed":
			return NewOpenAIWSClientCloseError(
				coderws.StatusInternalError,
				"upstream websocket proxy failed",
				errors.New("upstream websocket terminal failure"),
			)
		}
		logOpenAIWSV2Passthrough(
