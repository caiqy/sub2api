...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	if err != nil || len(responsesBody) == 0 {
		...
		responsesBody, _, err = buildOpenAIImagesOAuthPayload(parsed, upstreamModel)
	}
	targetURL := chatgptCodexURL
	if direct {
		targetURL = openAIImagesDirectTargetURL(parsed)
		responsesBody, err = sjson.SetBytes(responsesBody, "model", upstreamModel)
	} else if !gjson.GetBytes(responsesBody, "tools").Exists() {
		fallbackParsed := &OpenAIImagesRequest{Endpoint: parsed.Endpoint, ContentType: "application/json", N: 1}
		if err = parseOpenAIImagesJSONRequest(responsesBody, fallbackParsed); err == nil {
			applyOpenAIImagesDefaults(fallbackParsed)
			responsesBody, err = buildOpenAIImagesResponsesRequest(fallbackParsed, upstreamModel)
		}
	}
	if err != nil {
		return nil, err
	...
	upstreamReq, err := s.buildUpstreamRequestWithSourceBody(upstreamCtx, c, account, responsesBody, responsesBody, token, true, parsed.StickySessionSeed(), false)
	if err != nil {
		return nil, err
	}
