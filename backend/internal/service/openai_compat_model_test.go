...
func TestForwardAsAnthropic_ReusesOAuthCodexTurnState(t *testing.T) {
	...
	require.Equal(t, resolveConvergedSessionID(account), upstream.requests[1].Header.Get("session_id"))
	require.Equal(t, upstream.requests[1].Header.Get("session_id"), gjson.GetBytes(upstream.bodies[1], "client_metadata.session_id").String())
