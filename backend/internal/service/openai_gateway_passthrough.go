...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	// currentHandle is the live owner; retries replace it below.
	...
	var upstreamResponseCtx context.Context
	agentTaskRecoveryTried := agentIdentityTaskRecoveryWasTried(ctx)
	compactModelFallbackRetried := false
	rejectedFieldRetryState := openAIResponsesRejectedFieldRetryStateForRequest(c, body)
	body = nil //nolint:ineffassign // The replay handle owns the request across upstream waits.
	...
	for {
		...
		actualModel := strings.Clone(strings.TrimSpace(gjson.GetBytes(attemptBody, "model").String()))
		if actualModel == "" {
		...
		replacePassthroughHandle := func(nextBody []byte) error {
			nextHandle, _, handleErr := openAIRequestBodyHandleForBytes(currentHandle, nextBody)
			if handleErr != nil {
				return handleErr
			}
		...
		if reqStream {
			...
			if err != nil {
				lastAttemptBody, readErr := currentHandle.ReadAll()
				if readErr != nil {
					return nil, fmt.Errorf("read OpenAI passthrough fallback body: %w", readErr)
				}
				if retryBody, fallbackModel, retry := s.applyOpenAIPassthroughCompactFallbackFromSignal(
					c, account, requestedModel, lastAttemptBody, err, compactModelFallbackRetried, resp,
				); retry {
					if handleErr := replacePassthroughHandle(retryBody); handleErr != nil {
						return nil, handleErr
					}
					upstreamPassthroughModel = fallbackModel
					compactModelFallbackRetried = true
	...
	if finalBody, readErr := currentHandle.ReadAll(); readErr == nil {
		serviceTier = extractOpenAIServiceTierFromBody(finalBody)
	}
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	_, bodyHandleKnown := bodyHandleOrToken.(openAIMatchedRequestBodyHandle)
	...
	if account.UsesOpenAICodexProtocol() {
		...
		if compatMessagesBridge && promptCacheKey == "" {
			promptCacheKey = strings.TrimSpace(gjson.GetBytes(sourceBody, "prompt_cache_key").String())
		}
		...
		if account.GetCodexFingerprintMode() == codexFingerprintOff {
			if compatMessagesBridge && clientSessionID == "" {
			...
			if clientConversationID == "" && !compatMessagesBridge {
...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	retryableOnSameAccount := requestScopedTransient || (!shouldDisable && account.IsPoolMode() && account.IsPoolModeRetryableStatus(resp.StatusCode))
