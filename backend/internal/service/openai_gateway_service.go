...
type OpenAIForwardResult struct {
	...
	WebSearchCalls       int
	// UpstreamTerminalEvent is retained for local scheduler success accounting.
...
func (r *OpenAIForwardResult) UsageRecordSnapshot() *OpenAIForwardResult {
	...
	if len(r.ImageSizeBreakdown) > 0 {
		...
		for key, value := range r.ImageSizeBreakdown {
			out.ImageSizeBreakdown[key] = value
		}
	}
	return &out
}
...
func (s *OpenAIGatewayService) openAIStickyEnabled() bool {
	...
	// Simple mode keeps response continuations available even when ordinary
	// session sticky routing is disabled.
	if s.cfg.RunMode == config.RunModeSimple {
		return true
	}
	return s.cfg.GatewayControlRuntime().StickyOpenAIEnabled
}
...
func NewOpenAIGatewayService(
	...
	// enforceCodexIdentityHeaders is shared by all Codex egress paths.
	if cfg != nil {
