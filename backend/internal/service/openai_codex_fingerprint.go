...
const codexFingerprintCachedIDsContextKey = "codex_fingerprint_cached_ids"
...
func stagedCodexFingerprintIDs(c *gin.Context, account *Account) *codexFingerprintIDs {
	if c == nil || account == nil || !account.UsesOpenAICodexProtocol() || isOpenAIResponsesCompactPath(c) {
		return nil
	}
...
// codexFingerprintSeedFromSource accepts both the current seed call form and
// the pre-merge account call form used by compatibility tests.
func codexFingerprintSeedFromSource(source any) string {
	...
	case *Account:
		...
		seed, _ := codexFingerprintSeed(value.Extra)
		return seed
	default:
		return ""
	}
...
func resolveConvergedSessionID(source any) string {
	seed := codexFingerprintSeedFromSource(source)
	if seed == "" {
		return ""
	}
