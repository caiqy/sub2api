...
func (s *openAIWSStateStoreSpy) BindHTTPResponseOwner(context.Context, int64, string, int64, int64, time.Duration) error {
	return nil
}

func (s *openAIWSStateStoreSpy) GetHTTPResponseOwner(context.Context, int64, string) (int64, int64, bool, error) {
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_Weighted_AllowsGrokChatAccount(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_Weighted_PreservesGrokSessionSticky(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(10127)
	stickyAccount := Account{ID: 38003, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 100, GroupIDs: []int64{groupID}}
	preferredAccount := Account{ID: 38004, Platform: PlatformGrok, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}}
	cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{"openai:session_hash_grok_weighted": stickyAccount.ID}}
	cfg := newOpenAIStickyEnabledTestConfig()
	cfg.Gateway.OpenAIWS.SchedulerMode = "weighted"
	svc := &OpenAIGatewayService{
	...
	require.Zero(t, cache.deletedSessions["openai:session_hash_grok_weighted"])
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickyRuntimeBlockedAccountFallsBackWithoutRebinding(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "model specific transient cooldown",
			block: func(svc *OpenAIGatewayService, account *Account) {
				svc.openaiModelTransient = newOpenAIAccountModelTransientState(128)
				now := time.Now()
				svc.openaiModelTransient.recordFailure(account.ID, requestedModel, now)
				svc.openaiModelTransient.recordFailure(account.ID, requestedModel, now.Add(time.Millisecond))
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			sticky := Account{ID: 31011, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}}
			...
			sessionHash := "session_hash_runtime_blocked_" + tc.name
			sessionKey := "openai:" + sessionHash
			cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{sessionKey: sticky.ID}}
			acquiredIDs := []int64{}
			svc := &OpenAIGatewayService{
			...
			stickySelection, stickyReq, err := scheduler.selectBySessionHash(ctx, OpenAIAccountScheduleRequest{
				GroupID: &groupID, Platform: PlatformOpenAI, SessionHash: sessionHash, RequestedModel: requestedModel,
			})
			require.NoError(t, err)
...
func TestLayeredOpenAIAccountSchedulerSessionStickyRuntimeBlockedAccountFallsBackWithoutRebinding(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			sticky := Account{ID: 31021, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}}
			backup := Account{ID: 31022, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupID}}
			sessionHash := "layered_session_hash_runtime_blocked_" + tc.name
			sessionKey := "openai:" + sessionHash
			cache := &schedulerTestGatewayCache{sessionBindings: map[string]int64{sessionKey: sticky.ID}}
			acquiredIDs := []int64{}
			cfg := newOpenAIStickyEnabledTestConfig()
...
func TestDefaultOpenAIAccountScheduler_WeightedStickyFallbackAllowsGrok(t *testing.T) {
