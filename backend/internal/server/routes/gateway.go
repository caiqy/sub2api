...
func RegisterGatewayRoutes(
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, usageDetailCapture, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	r.GET("/responses", bodyLimit, clientRequestID, usageDetailCapture, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, compositeWebSocketTarget, requireGroupAnthropic, func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformGrok {
	...
	codexDirect.Use(bodyLimit, clientRequestID, usageDetailCapture, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
...
func compositeTargetPlatformMiddleware(resolver *service.EffectiveGatewayRouteResolver) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		clientModel, _ := requestmodel.JSONModelPathForRoute(routePath, body)
		if clientModel == "" {
			clientModel = requestmodel.FromBodyForRoute(routePath, c.GetHeader("Content-Type"), body)
		}
		clientModel = strings.Clone(clientModel)
