...
// migrations 193/234 回归：groups 触发器的 durable 失效监视清单必须覆盖利润控制
...
func TestAuthCacheInvalidationTrigger_GroupPolicyColumns(t *testing.T) {
	...
	for name, update := range map[string]string{
		...
		"force_openai_fast":               "force_openai_fast = true",
		"free_openai_fast":                "free_openai_fast = true",
		"max_reasoning_effort_over_limit": "max_reasoning_effort_over_limit = 'deny'",
	} {
...
func TestAuthCacheInvalidationTrigger_PublicGroupRestriction(t *testing.T) {
	ctx := context.Background()
	suffix := time.Now().UnixNano()
	group := mustCreateGroup(t, integrationEntClient, &service.Group{
		Name: fmt.Sprintf("public-restriction-trigger-group-%d", suffix), Platform: service.PlatformOpenAI, RateMultiplier: 1,
	})
	user := mustCreateUser(t, integrationEntClient, &service.User{
		Email: fmt.Sprintf("public-restriction-trigger-%d@example.com", suffix), Concurrency: 5,
	})
	groupID := group.ID
	keyValue := fmt.Sprintf("sk-public-restriction-trigger-%d", suffix)
	apiKeyRepo := NewAPIKeyRepository(integrationEntClient, integrationDB)
	key := &service.APIKey{UserID: user.ID, GroupID: &groupID, Key: keyValue, Name: "public-restriction-trigger", Status: service.StatusActive}
	require.NoError(t, apiKeyRepo.Create(ctx, key))
	...
	t.Cleanup(clear)
	t.Cleanup(func() {
		_, err := integrationDB.ExecContext(ctx, "DELETE FROM user_allowed_groups WHERE user_id = $1 AND group_id = $2", user.ID, group.ID)
		require.NoError(t, err)
		_, err = integrationDB.ExecContext(ctx, "DELETE FROM api_keys WHERE id = $1", key.ID)
		require.NoError(t, err)
		_, err = integrationDB.ExecContext(ctx, "DELETE FROM users WHERE id = $1", user.ID)
	...
	require.NoError(t, err)
	require.Equal(t, 1, count(), "restrict_public_groups 变更必须入队")
	clear()

	_, err = integrationDB.ExecContext(ctx,
		"INSERT INTO user_allowed_groups (user_id, group_id) VALUES ($1, $2)", user.ID, group.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "受限用户的公开分组授权变更必须入队")
}
