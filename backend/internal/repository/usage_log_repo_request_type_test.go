...
func TestUsageLogRepositoryCreateSyncRequestTypeAndLegacyFields(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO usage_logs").
		WithArgs(
			...
			sqlmock.AnyArg(), // request_body_size
			sqlmock.AnyArg(), // response_body_size
...
func TestPrepareUsageLogInsert_PersistsNativeCompactionV2WithoutChangingRequestType(t *testing.T) {
	...
	require.Equal(t, sql.NullString{String: requestedReasoningEffort, Valid: true}, prepared.args[48])
...
func TestUsageLogRepositoryGetStatsWithFiltersAppliesMismatchFilterToEndpointAggregations(t *testing.T) {
	for _, mismatch := range []bool{true, false} {
		t.Run(fmt.Sprintf("mismatch_%t", mismatch), func(t *testing.T) {
			...
			mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE " + condition + ".*GROUP BY GROUPING SETS").
				WillReturnRows(sqlmock.NewRows([]string{
					"inbound_grouped", "upstream_grouped", "inbound_endpoint", "upstream_endpoint",
					"requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
					"cost", "actual_cost", "account_cost", "avg_duration_ms",
					...
					AddRow(0, 1, "/v1/responses", nil, int64(0), int64(0), int64(0), int64(0), int64(0), 0.0, 0.0, 0.0, 0.0).
					AddRow(1, 0, nil, "/v1/responses", int64(0), int64(0), int64(0), int64(0), int64(0), 0.0, 0.0, 0.0, 0.0).
					AddRow(0, 0, "/v1/responses", "/v1/responses", int64(0), int64(0), int64(0), int64(0), int64(0), 0.0, 0.0, 0.0, 0.0))
