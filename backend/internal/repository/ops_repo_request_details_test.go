...
func TestListRequestDetailsIncludesModelTriplet(t *testing.T) {
	...
	mock.ExpectQuery(`(?s)WITH combined AS.*requested_model.*upstream_model.*FROM combined`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			"duration_ms", "first_token_ms", "status_code", "error_id", "phase", "severity", "message",
			"user_id", "api_key_id", "account_id", "group_id", "stream",
		}).AddRow(
			"error", time.Now(), "req_1", "openai", "gpt-5", "public-alias", "gpt-5.2",
			nil, nil, 502, int64(8), "upstream", "P1", "upstream failed",
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rows := sqlmock.NewRows([]string{
				"kind", "created_at", "request_id", "platform", "model", "requested_model", "upstream_model", "duration_ms", "first_token_ms",
				"status_code", "error_id", "phase", "severity", "message", "user_id", "api_key_id", "account_id", "group_id", "stream",
			}).
				AddRow("success", start, "req-slow", "openai", "gpt-5.5", "gpt-5.5", "gpt-5.5", 12000, 800, nil, nil, nil, nil, nil, 1, 2, 3, 4, true).
				AddRow("error", start, "req-zero", "openai", "gpt-5.5", "gpt-5.5", "gpt-5.5", 9000, 0, 502, 5, "upstream", "error", "failed", 1, 2, 3, 4, true).
				AddRow("success", start, "req-missing", "openai", "gpt-5.5", "gpt-5.5", "gpt-5.5", 5000, nil, nil, nil, nil, nil, nil, 1, 2, 3, 4, false)
