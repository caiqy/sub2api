...
func (s *ProxyExpirySuite) TestUpdateRejectsStaleSnapshot() {
	id := s.mkProxy("concurrent-update", service.FallbackModeNone, nil, nil)
	first, err := s.repo.GetByID(s.ctx, id)
	s.Require().NoError(err)
	stale, err := s.repo.GetByID(s.ctx, id)
	s.Require().NoError(err)

	first.Name = "first-update"
	s.Require().NoError(s.repo.Update(s.ctx, first))
	stale.Host = "127.0.0.2"
	err = s.repo.Update(s.ctx, stale)
	s.Require().True(errors.Is(err, service.ErrProxyUpdateConflict))

	current, err := s.repo.GetByID(s.ctx, id)
	s.Require().NoError(err)
	s.Require().Equal("first-update", current.Name)
	s.Require().Equal("127.0.0.1", current.Host)
}
