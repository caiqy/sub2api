...
func provideCleanup(
	...
	return func() {
		...
		producerSteps := []cleanupPhase{
			...
			{name: "OpenAIQuotaAutoResetService", run: func(context.Context) error {
			...
			{name: "SubscriptionCacheInvalidationWorker", run: func(context.Context) error {
			...
			{name: "OpenAICodexVersionSyncService", run: func(context.Context) error {
				if codexVersionSync != nil {
					codexVersionSync.Stop()
				}
				return nil
			}},
			{name: "CNProviderBalanceCheckService", run: func(context.Context) error {
